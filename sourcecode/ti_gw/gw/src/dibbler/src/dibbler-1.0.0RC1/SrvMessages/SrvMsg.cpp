...
using namespace std;
extern string fqdn_addr; // UNIHAN ADD , PROD00199235 PROD00197285

// UNIHAN ADD START FOR PROD00207932
#define OPTION_FILE_PATH 256
// UNIHAN ADD END 
    ...
    :TMsg(iface, addr, buf, bufSize), forceMsgType_(0), physicalIface_(iface)
{
    ...
    int pos=0;
    // UNIHAN ADD START FOR PROD00207932
    char output_opt_file[OPTION_FILE_PATH] = {0};
    unsigned char *opt_val = NULL;
    int i = 0;
    FILE *opt_file = NULL;

    if( MsgType == REQUEST_MSG || MsgType == CONFIRM_MSG )
    {
        snprintf( output_opt_file , sizeof( output_opt_file ), "%s%s%s" ,DHCPV6_OPTION_FILE_PATH ,DHCPV6_OPTION_FILE ,(TIPv6Addr *)addr->getPlain() );
        opt_file = fopen( output_opt_file , "w+");
        if( !opt_file )
            Log(Error) << "OPTION FILE OPEN FAILED" << LogEnd ;
    }
    // UNIHAN ADD END
    ...
    while (pos<bufSize)	{
        ...
        case OPTION_NEW_TZDB_TIMEZONE:
            ...
            break;
        case OPTION_FQDN:
            // UNIHAN ADD START , PROD00199235 PROD00197285
            fqdn_addr.assign(addr->getPlain());
            ptr = new TSrvOptFQDN(buf+pos, length, this);
            // UNIHAN ADD END , PROD00199235 PROD00197285
            break;
        ...
        // UNIHAN ADD START FOR PROD00207932
        {
            if( MsgType == REQUEST_MSG || MsgType == CONFIRM_MSG )
            {
                if( opt_file )
                {
                    fprintf( opt_file , "%s %d:" , "option" ,code ); 
                    for( i = 0 ; i < length ; i++ ) 
                    {
                        fprintf( opt_file , "%02X" , opt_val[i] );
                    }
                    fprintf( opt_file , "\n" );
                }
                else
                {
                    Log(Error) << "OPTION FILE OPEN FAILED : " << output_opt_file << LogEnd;
                }
            }
...
//UNIHAN MODIFY
            ...
            SPtr<TDUID> ClntDuid = clientMsg->getClientDUID();
            SrvAddrMgr().setDeviceAssociationToAddrClientList(ClntDuid, BROADBAND_V6SUBOPT_DEVICE_MANUFACTUREROUI, v->get_Broadband_DeviceManufacturerOUI());
            SrvAddrMgr().setDeviceAssociationToAddrClientList(ClntDuid, BROADBAND_V6SUBOPT_DEVICE_SERIALNUMBER, v->get_Broadband_DeviceSerialNumber());
            SrvAddrMgr().setDeviceAssociationToAddrClientList(ClntDuid, BROADBAND_V6SUBOPT_DEVICE_PRODUCTCLASS, v->get_Broadband_DeviceProductClass());
#else
...
/*SERCOMM MODIFY END*/
    ...
    {
        if (((*gen)->getOptType() == OPTION_DNS_SERVERS) || 
            reqOpts->isOption( (*gen)->getOptType())) /*SEROCMM MODIFY*/
        {
...
extern bool runTimeConfNeedUpdate;
bool TSrvMsg::appendBroadBandVendorSpec(bool isGetDeviceIdentity, bool isGetDslForumString)
{
    if(runTimeConfNeedUpdate)
    {
        this->updateBroadBandVendorSpec();
    }
    unsigned int enterpriseNumber;
    unsigned int subOptionCode;
    unsigned int cwmpEnabled = false;
    std::stringstream tempStream;
    std::string subOptionData;
    std::string vendorSpecInfoBuffer;
    SPtr<TOpt> SPtrBroadBandVendorSpec;

    cwmpEnabled = this->gwdb_config.tr69_enable;
    if (cwmpEnabled == false || cwmpEnabled == -1)
    {
        /* Don't packet option17, if is unable to confirm whether the CWMP is enabled.*/
        return false;
    }
    ...
    {
        /* Don't packet option17, if device identity and 'dslforum.org' are not received.*/
        return false;
    }
    /* Set 32-bits enterprise number */
    enterpriseNumber = IANA_ENTERPRISE_BROADBAND_FORUM;
    vendorSpecInfoBuffer += ((enterpriseNumber >> 24) & 0xFF);
    vendorSpecInfoBuffer += ((enterpriseNumber >> 16) & 0xFF);
    vendorSpecInfoBuffer += ((enterpriseNumber >> 8) & 0xFF);
    vendorSpecInfoBuffer += (enterpriseNumber & 0XFF);
    /* Reply ACS parameters */
    if (isGetDslForumString == true)
    {
        /* ACS URL */
        subOptionCode = BROADBAND_V6SUBOPT_ACS_URL;
        subOptionData.clear();
        if (this->gwdb_config.tr69_v6_acs_url != NULL)
        {
              subOptionData = this->gwdb_config.tr69_v6_acs_url;
        }

        /* Set 16-bits sub option code */
        vendorSpecInfoBuffer += ((subOptionCode >> 8) & 0XFF);
        vendorSpecInfoBuffer += (subOptionCode & 0XFF);
        /* Set 16-bits sub option length */
        vendorSpecInfoBuffer += ((subOptionData.length() >> 8) & 0XFF);
        vendorSpecInfoBuffer += (subOptionData.length() & 0XFF);
        /* Set sub option data */
        vendorSpecInfoBuffer += subOptionData;

        /* Provisioning code */
        subOptionCode = BROADBAND_V6SUBOPT_PROVISIONING_CODE;
        subOptionData.clear();
        if (this->gwdb_config.tr69_v6_provision_code != NULL)
        {
            subOptionData = this->gwdb_config.tr69_v6_provision_code;
        }
        ...
        /* CWMP retry minimum wait interval */
        subOptionCode = BROADBAND_V6SUBOPT_CWMP_RETRY_MINIMUM_WAIT_INTERVAL;
        subOptionData.clear();
        tempStream.str("");
        tempStream.clear();
        if (this->gwdb_config.tr69_v6_retry_wait_interval != -1)
        {
            tempStream << this->gwdb_config.tr69_v6_retry_wait_interval;
        }
        subOptionData = tempStream.str();
        ...
        /* CWMP retry interval multiplier */
        subOptionCode = BROADBAND_V6SUBOPT_CWMP_RETRY_INTERVAL_MULTIPLIER;
        subOptionData.clear();
        tempStream.str("");
        tempStream.clear();
        if (this->gwdb_config.tr69_v6_retry_interval_multi != -1)
        {
            tempStream << this->gwdb_config.tr69_v6_retry_interval_multi;
        }
        subOptionData = tempStream.str();
    ...
    {
        /* Gateway OUI */
        subOptionCode = BROADBAND_V6SUBOPT_GW_MANUFACTUREROUI;
        subOptionData.clear();
        if (this->gwdb_config.vendor_id != NULL)
        {
            subOptionData = this->gwdb_config.vendor_id;
        }
        ...
        subOptionCode = BROADBAND_V6SUBOPT_GW_SERIALNUMBER;
        subOptionData.clear();
        if (this->gwdb_config.serial_number != NULL)
        {
            subOptionData = this->gwdb_config.serial_number;
        }
        ...
        /* Gateway product class */
        subOptionCode = BROADBAND_V6SUBOPT_GW_PRODUCTCLASS;
        subOptionData.clear();
        if (this->gwdb_config.hw_model != NULL)
        {
            subOptionData = this->gwdb_config.hw_model;
        }
    ...
    SPtrBroadBandVendorSpec = new TOptVendorSpecInfo( OPTION_VENDOR_OPTS, const_cast<char *>(vendorSpecInfoBuffer.c_str()), vendorSpecInfoBuffer.length(), this);
    Options.push_back(SPtrBroadBandVendorSpec);
    tempStream.str("");
    tempStream.clear();
...
#define SSCANF_COUNT                           2
bool TSrvMsg::updateBroadBandVendorSpec()
{
    runTimeConfNeedUpdate = false;
    std::string runtime_conf = SrvCfgMgr().getRunTimeConfName();
    Log(Crit) << "Received SIGUSR1 - updating dibbler GWDB runtime conf" << LogEnd;
    FILE *fp = NULL;
    int errcode = 0;
    int ret = 0;
    char buf[MAX_FILE_STRING_LENGTH]={'\0'};
    char tag[MAX_TEMP_STRING_LENGTH]={'\0'};
    char value[MAX_TEMP_STRING_LENGTH]={'\0'};

    fp = fopen(runtime_conf.c_str(), "r");
    if (!fp)
    {
        Log(Crit) << "Update dibbler GWDB runtime conf fail" << LogEnd;
        return false;
    }
    ...
    {
        memset(tag, '\0', MAX_TEMP_STRING_LENGTH);
        memset(value, '\0', MAX_TEMP_STRING_LENGTH);
        if(sscanf(buf,"%s %s",tag,value) != SSCANF_COUNT)
        {
            continue;
        }

        if (!strcasecmp(tag, "tr69_enable"))
        {
            this->gwdb_config.tr69_enable = atoi(value);
            continue;
        }

        if (!strcasecmp(tag, "tr69_v6_acs_url"))
        {
            this->gwdb_config.tr69_v6_acs_url = strdup(value);
            continue;
        }

        if (!strcasecmp(tag, "tr69_v6_provision_code"))
        {
            this->gwdb_config.tr69_v6_provision_code = strdup(value);
            continue;
        }

        if (!strcasecmp(tag, "tr69_v6_retry_wait_interval"))
        {
            this->gwdb_config.tr69_v6_retry_wait_interval = atoi(value);
            continue;
        }

        if (!strcasecmp(tag, "tr69_v6_retry_interval_multi"))
        {
            this->gwdb_config.tr69_v6_retry_interval_multi = atoi(value);
            continue;
        }

        if (!strcasecmp(tag, "vendor_id"))
        {
            this->gwdb_config.vendor_id = strdup(value);
            continue;
        }

        if (!strcasecmp(tag, "serial_number"))
        {
            this->gwdb_config.serial_number = strdup(value);
            continue;
        }

        if (!strcasecmp(tag, "hw_model"))
        {
            this->gwdb_config.hw_model = strdup(value);
            continue;
        }
        memset(buf, '\0', MAX_FILE_STRING_LENGTH);
    }
    
    fclose(fp);
    Log(Crit) << "Received SIGUSR1 - updating dibbler GWDB runtime conf Finished." << LogEnd;
    return true;
}
