...
/*SERCOMM ADD START*/
#define AVAHI_AUTOIPD_RECORD    "/var/tmp/avahiAutoipdRecord"
/*SERCOMM ADD END*/
...
static int append_rdata(AvahiDnsPacket *p, AvahiRecord *r) {
    assert(p);
    assert(r);
    char ipStr[AVAHI_ADDRESS_STR_MAX] = {0}; //SERCOMM ADD
    FILE *autoipd; //SERCOMM ADD
    ...
    switch (r->key->type) {
        ...
        {
                /*SERCOMM ADD START*/
                if ((autoipd = fopen(AVAHI_AUTOIPD_RECORD, "r")))            
                {
                    avahi_log_debug("%s[%d]\n",__FUNCTION__,__LINE__); 
                                    
                    char serverip[AVAHI_ADDRESS_STR_MAX], avahip[AVAHI_ADDRESS_STR_MAX], brname[AVAHI_ADDRESS_STR_MAX];

                    while (fscanf(autoipd, "%s %s %s\n", serverip, avahip, brname) != EOF)
                    {
                        avahi_log_debug("servip=%s avahip=%s\n", serverip, avahip); 
                        if (inet_ntop(AF_INET, &r->data.a.address, ipStr, INET_ADDRSTRLEN))
                        {
                            avahi_log_debug("r->data.a.address=%s\n", ipStr); 
                            if (strcmp(serverip, ipStr) == 0)
                            {
                                if (inet_pton(AF_INET, avahip, &r->data.a.address) <= 0) 
                                {
                                    avahi_log_debug("append_rdata inet_pton fail\n"); 
                                }
                                break;
                            }
