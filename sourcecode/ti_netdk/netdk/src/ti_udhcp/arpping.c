...
int arpping(u_int32_t yiaddr, u_int32_t ip, unsigned char *mac, char *interface, unsigned char *hwaddr)
{
    ...
    if (setsockopt(s, SOL_SOCKET, SO_BINDTODEVICE,interface, strlen(interface) + 1) == -1)
    {
		LOG(LOG_ERR, "Could not BINDTODEVICE on raw socket");
		close(s);
		return -1;
    }
    ...
    memcpy((void *)&arp.sInaddr, (void *)&ip, sizeof(u_int32_t));
	//*((u_int *) arp.sInaddr) = ip;			/* source IP address */
	memcpy(arp.sHaddr, mac, 6);			/* source hardware address */
    /* This doesnt seem to be working well on Volcano */
	//*((u_int *) arp.tInaddr) = yiaddr;		/* target IP address */
    memcpy((void *)&arp.tInaddr, (void *)&yiaddr, sizeof(u_int32_t));
