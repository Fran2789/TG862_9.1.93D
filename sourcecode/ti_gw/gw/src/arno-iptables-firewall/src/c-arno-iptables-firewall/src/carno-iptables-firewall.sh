...
#**************************************************************
export ENV_FILE=/fss/gw/usr/local/share/arno-iptables-firewall/cenvironment.sh
...
{
  #SERCOMM ADD
  #SERCOMM NOTE: patch revision 121, 178 (removed in revision 178)
  # UNIHAN ADD START
  #echo "true" > "/var/tmp/getfwstatus"
  # UNIHAN ADD END
  #SERCOMM ADD END
...
#  setup_kernel_settings;
  main_restart_c;
#SERCOMM ADD
#SERCOMM NOT:  patch revision 178
#UNIHAN ADD START
# Keep this section at the end of function
# ARRIS TODO : add variable to arris shmemdb instead - set within carno
  echo "false" > /var/tmp/getfwstatus
#UNIHAN ADD END
#SERCOMM ADD END
}
...
# ARRIS MOD BEGIN : Just send the main command to carno so it can 
#                   perform the whole sequence in one call
...
#                        esac
#                        show_applied;;
#  'force-reload' )      config_check;
#                        setup_hostblock_chain;
#                        show_applied;;
#  'stop' )              show_stop;
#                        main_stop;
#                        show_disabled;;
#  'stop-block' )        show_stop;
#                        main_stop block;
#                        show_stop_blocked;;
#  'status' )            shift;
#                        show_status $@;;
...
# let's kill the symlinks, and just spawn a single process here - argv[1] is what we want to do

# Check commandline supplied argument:
case "$1" in
  'start' )             case $(check_for_base_chains) in
                          no)    c_arno-iptables-firewall start_dostart $2;;
                          yes)   c_arno-iptables-firewall start_dorestart $2;;
                          other) c_arno-iptables-firewall start_dostopstart $2;;
                        esac;;
  'restart' )           case $(check_for_base_chains) in
                          no)    c_arno-iptables-firewall restart_dostart $2;;
                          yes)   c_arno-iptables-firewall restart_dorestart $2;;
                          other) c_arno-iptables-firewall restsart_dostopstart $2;;
                        esac;;
  'force-reload' )      c_arno-iptables-firewall force-reload $2;;
  'stop' )              c_arno-iptables-firewall stop $2;;
  'stop-block' )        c_arno-iptables-firewall stop-block $2;;
  'status' )            shift;
                        show_status $@;;
  'check-conf' )        c_arno-iptables-firewall check-conf $2;;
  * )                   printf "\033[40m\033[1;31mERROR: Bad or missing parameter(s)\033[0m\n" >&2;
                        show_help;;
esac
