...
/* Includes Intel Corporation's changes/modifications dated: [Jan 9, 2013]. 
Changed/modified portions - Copyright © [2013], Intel Corporation. 
Modification: add unique error codes for "file not found" and "server not found"*/ 
...
#include "libbb.h"
#include <net/if.h>   /* for struct ifreq */ /* UNIHAN ADD */

/*we define 2 unique error codes for SW_DL process to identify if the HTTP server of file were not found*/
#define SW_DL_ERROR_CODE_SERVER_NOT_FOUND   4
#define SW_DL_ERROR_CODE_FILE_NOT_FOUND   8
...
struct globals {
	...
	bb_progress_t pmt;
#endif
	char *iface;   /* interface to bind on */ /* UNIHAN ADD */
        char *dir_prefix;
...
enum {
	...
	WGET_OPT_INTERFACE  = (1 << 12),   /* UNIHAN ADD */
};
...
static FILE *open_socket(len_and_sockaddr *lsa)
{
	...
	xfunc_error_retval = SW_DL_ERROR_CODE_SERVER_NOT_FOUND; /* If we are unalbe to "connect" to server, we define a unique
							error code '4' for Software Download process to know about it*/
	/* UNIHAN MOD START */
	//fp = fdopen(xconnect_stream(lsa), "r+");
	s = xsocket(lsa->u.sa.sa_family, SOCK_STREAM, 0);
	if (G.iface != NULL)
	{
        setsockopt_bindtodevice(s, G.iface);
	}
	xconnect(s, &lsa->u.sa, lsa->len);

	fp = fdopen(s, "r+");
	/* UNIHAN MOD END */
	if (fp == NULL)
		bb_perror_msg_and_die(bb_msg_memory_exhausted);
	xfunc_error_retval = EXIT_FAILURE; /*we were able to connect to server, error code is back to default*/
	return fp;
}
...
*/
		...
		case 206:
			if (G.beg_range)
				break;
			/* fall through */
		case 404: /* file not found - we define a unique error code '8' for Software Download process 
					 identify this error*/
			xfunc_error_retval = SW_DL_ERROR_CODE_FILE_NOT_FOUND;
