...
int do_mem_md ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	do {
		...
		for (i=0; i<linebytes; i+= size) {
#ifdef CONFIG_EXTENDED_FLASH
            switch_res = process_flash_bank(NULL, addr, size, NULL, 1);
            if (switch_res == EMM_HIGH)
            {
                addr = (uchar*) addr - FLASH_BORDER_OFFSET_16MB;
            }
            /*If we will read not alligned data, for example
                md.w 0x4800ffef 2 (read 2 x 2 bytes) or 
                md 0x4800ffed 1 (read 1 x 4 bytes), 
            then md will crash (both old and this versions), 
            so on partial reading (reading cross the banks) each chunk of data
            (1, 2 or 4 bytes) will be either in low or high bank*/
#endif
			if (size == 4) {
...
int do_mem_cmp (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
...
#endif 
#ifdef CONFIG_EXTENDED_FLASH
    int switch_res1 = 0, switch_res2 = 0;
#endif
	...
	*/
	...
	while (count-- > 0) {
		if (size == 4) {
#ifdef CONFIG_EXTENDED_FLASH
            /*Assume that address alligned properly, i.e. occupies one bank*/
            switch_res1 = process_flash_bank(NULL, addr1, sizeof(ulong), NULL, 1);
            if (switch_res1 == EMM_HIGH)
                addr1 = (uchar*) addr1 - FLASH_BORDER_OFFSET_16MB;
#endif
			ulong word1 = *(ulong *)addr1;
#ifdef CONFIG_EXTENDED_FLASH
            switch_res2 = process_flash_bank(NULL, addr2, sizeof(ulong), NULL, 1);
            if (switch_res2 == EMM_HIGH)
                addr2 = (uchar*) addr2 - FLASH_BORDER_OFFSET_16MB;
#endif
...
#ifdef CONFIG_EXTENDED_FLASH
            switch_res1 = process_flash_bank(NULL, addr1, sizeof(ushort), NULL, 1);
            if (switch_res1 == EMM_HIGH)
                addr1 = (uchar*) addr1 - FLASH_BORDER_OFFSET_16MB;
#endif
			ushort hword1 = *(ushort *)addr1;
#ifdef CONFIG_EXTENDED_FLASH
            switch_res2 = process_flash_bank(NULL, addr2, sizeof(ushort), NULL, 1);
            if (switch_res2 == EMM_HIGH)
                addr2 = (uchar*) addr2 - FLASH_BORDER_OFFSET_16MB;
#endif
...
#ifdef CONFIG_EXTENDED_FLASH
            switch_res1 = process_flash_bank(NULL, addr1, sizeof(uchar), NULL, 1);
            if (switch_res1 == EMM_HIGH)
                addr1 = (uchar*) addr1 - FLASH_BORDER_OFFSET_16MB;
#endif
			u_char byte1 = *(u_char *)addr1;
#ifdef CONFIG_EXTENDED_FLASH
            switch_res2 = process_flash_bank(NULL, addr2, sizeof(uchar), NULL, 1);
            if (switch_res2 == EMM_HIGH)
                addr2 = (uchar*) addr2 - FLASH_BORDER_OFFSET_16MB;
#endif
...
int do_mem_cp ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	*/
	...
	while (count-- > 0) {
#ifdef CONFIG_EXTENDED_FLASH
        /*Assume that data aligned properly*/
        switch_res = process_flash_bank(NULL, addr, 0, NULL, 1);
        if (switch_res == EMM_HIGH)
            addr = (uchar*) addr - FLASH_BORDER_OFFSET_16MB;
#endif
		if (size == 4)
