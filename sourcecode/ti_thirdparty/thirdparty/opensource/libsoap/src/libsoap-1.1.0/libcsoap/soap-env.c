...
/* UNIHAN ADD START */
void soap_env_new_with_doc(const char *urn,  xmlChar *buffer, const char *method, int version)
{
    if (!strcmp(urn, ""))
    {
#ifdef USE_XMLSTRING
        if (version == 1999)
        {
            xmlStrPrintf(buffer, 1054, BAD_CAST _SOAP_MSG_TEMPLATE_EMPTY_TARGET_,
                    soap_xsi_ns_1999, soap_xsd_ns_1999, soap_env_ns,
                    BAD_CAST method, BAD_CAST urn, BAD_CAST method);
        }
        else
        {
            xmlStrPrintf(buffer, 1054, BAD_CAST _SOAP_MSG_TEMPLATE_EMPTY_TARGET_,
                    soap_xsi_ns_2001, soap_xsd_ns_2001, soap_env_ns,
                    BAD_CAST method, BAD_CAST urn, BAD_CAST method);
        }
#else
        if (version == 1999)
    ...
    herror_t
soap_env_new_with_method2(const char *urn, const char *method, SoapEnv ** out, int version)
{
    xmlDocPtr env;
    xmlChar buffer[1054];
    ...
    soap_env_new_with_doc(urn,  buffer, method, version);

    if (!(env = xmlParseDoc(buffer)))
        return herror_new("soap_env_new_with_method2",
                XML_ERROR_PARSE, "Can not parse xml");
...
soap_env_new_with_response(SoapEnv * request, SoapEnv ** out)
{
    ...
    /* UNIHAN MOD START */
    ret = soap_env_new_with_method2(urn, res_method, out, request->soap_format_version);
    /* UNIHAN MOD END */
...
soap_env_new_with_method(const char *urn, const char *method, SoapEnv ** out)
{
    ...
    /* UNIHAN MOD START */
    soap_env_new_with_doc(urn,  buffer, method, 2001);
#if 0
    if (!strcmp(urn, ""))
    {
#ifdef USE_XMLSTRING
        xmlStrPrintf(buffer, 1054, BAD_CAST _SOAP_MSG_TEMPLATE_EMPTY_TARGET_,
                soap_xsi_ns, soap_xsd_ns, soap_env_ns,
                BAD_CAST method, BAD_CAST urn, BAD_CAST method);
#else
        sprintf(buffer, _SOAP_MSG_TEMPLATE_EMPTY_TARGET_,
                soap_xsi_ns, soap_xsd_ns, soap_env_ns,
