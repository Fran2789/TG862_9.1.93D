...
/*Get scope_id and local link address from /proc/net/if_inet6  */
int get_ipv6_if_info( char* if_name ,int* if_id, struct in6_addr* localaddr )
{
	char line[80];
	FILE *fp;
	char ip6addr[80];
	...
	while (fgets(line, 80, fp) != NULL) {
		sscanf(line,"%s %s %*s %s %*s %s",ip6addr, if_index, scope, name);
		
		if( scope[0]=='2' // Link Local
		    && strcmp(name,if_name)==0 ) {
			*if_id = strtol(if_index, NULL, 16);

			fill_ip6addr(localaddr,ip6addr);
...
void conv_qname (char *qname, struct in6_addr *a6) 
{
        int i;
	char temp[16];

        for (i = 15; i >= 0; i--) {
		//sprintf(temp,"\001%x\001%x", (a6->s6_addr[i] & 0x0F), (a6->s6_addr[i]>1 & 0x0F)  );
		sprintf(temp,"\001%x\001%x", a6->s6_addr[i] & 0x0F , (a6->s6_addr[i]>>4) & 0x0F  );
		strcat(qname,temp);
        }

	sprintf(temp,"\003ip6\004arpa\0");
	strcat(qname,temp);

        return;
}

int fill_ip6addr(struct in6_addr *in6addr, char *ip6str)
{
	int i=0;
	char temp[8];
...
int send_ipv6_query(int sock, struct in6_addr dest_addr, struct in6_addr target_addr, uint32_t rtt_base) 
{
        struct nbname_ipv6_request request;
	...
	struct timeval tv;
        ...
        dest_sockaddr.sin6_port = htons(LLMNR_PORT);
	memcpy( &dest_sockaddr.sin6_addr , &dest_addr,sizeof(struct in6_addr));
	...
	conv_qname(request.question_name , &target_addr );
	
	request.question_type = htons(0x0C); //RT_PTR
        request.question_class = htons(0x01);
	...
	status = sendto(sock, (char*)&request, sizeof(request), 0,
                (struct sockaddr_in *)&dest_sockaddr, sizeof(dest_sockaddr));
	if(status==-1) {
//	        snprintf(errmsg, 80, "%s\tSendto failed", inet_ntoa(dest_addr));
	        return(-1);
        };
