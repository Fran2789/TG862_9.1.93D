...
#define LINE_LENGTH 80
  if(interface && have_inet6){
	...
	while (fgets(line, LINE_LENGTH, fp) != NULL) {
		...
		sa_recv.sin6_scope_id = strtol(if_index, NULL, 16);
		fill_ip6addr(&(sa_recv.sin6_addr), ip6addr);
	}
	fclose(fp);
	if(bind(sock,(struct sockaddr *) &sa_recv,sizeof(sa_recv)) == -1) {
    		closesocket(sock);
		return -1;
	}
  }
  ...
  if(interface && !have_inet6){
	...
	if (setsockopt(sock, SOL_SOCKET, SO_BINDTODEVICE,(char *)&ifr, sizeof(ifr)) < 0) {
		printf("udns: setsockopt, SO_BINDTODEVICE to %s error\n", ifr.ifr_name);
	}
...
static int dns_next_srch(struct dns_ctx *ctx, struct dns_query *q) {
  ...
  for(;;) {
	...
	// Han added domain name length shouldn't be 1;
//printf("dnl=%x\n",dnl);
    if(dnl==1)
      return 0;
