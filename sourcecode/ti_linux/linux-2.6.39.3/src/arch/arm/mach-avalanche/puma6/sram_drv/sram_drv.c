...
static int sram_bootparams_read_proc(char *page, char **start, off_t off, int count,int *eof, void *data_unused);
...
// ARRIS ADD
#define PROC_BP_CEFDK_BOOT_ID                   "  CEFDK BOOT ID ................ "
// END ARRIS ADD
...
static boot_param_t BootParams[MAX_BOOT_PARAMS] = 
{
    ...
    // ARRIS ADD
    [CEFDK_BOOT_ID_ID] =              { PROC_BP_CEFDK_BOOT_ID,               sizeof(int ),GET_SRAM_ADDR(CEFDK_BOOT_ID)},  
    // END ARRIS ADD
    };
...
/**************************************************************************************/
/*! \fn static int sram_bootparams_read_proc (char *page, char **start, 
...
static int sram_bootparams_read_proc (char *page, char **start, off_t off, int count, int *eof, void *data_unused)
{
    ...
    for (paramId = 0; ((paramId < MAX_BOOT_PARAMS) && (!stopWrite)); paramId++) 
    {
        ...
        case (sizeof(char)):
            ...
            len = proc_snprintf(&page_off, &page_size,off,&curr_offset, "%s 0x%.2X\n", BootParams[paramId].name, paramVal);        
            break; 
        case (sizeof(int)):
            /* write the next boot param in the int format */
            len = proc_snprintf(&page_off, &page_size,off,&curr_offset, "%s 0x%.8X\n", BootParams[paramId].name, paramVal);
            break;
        }

        /* If proc_snprintf didn't succeed stop to write*/
        if (len == -1)
        {
            stopWrite = 1;
        }
        /* Else add the length that was written to the buffer */
        else
        {
            ret += len;
        }
...
static int __init sram_drv_init(void)
{
    ...
    printk(KERN_INFO "SRAM: Intel(R) SRAM driver built on %s @ %s\n", __DATE__, __TIME__);
    ...
    procFile = create_proc_entry(SRAM_BOOTRAM_PROC_NAME, 0644, NULL);
    ...
    procFile->read_proc = sram_bootparams_read_proc;
