...
SME_BEGIN_LEAF_STATE_DEF_P(PowerDown, PowerDownEntry, PowerDownExit)
	...
	SME_ON_EVENT_WITH_GUARD(EXT_EVENT_ID_POWER, Guard1_func, OnPowerDownEXT_EVENT_ID_POWER, OrthoState) // Test Case: transit to an orthogonal state
SME_END_STATE_DEF
...
SME_BEGIN_SUB_STATE_DEF_P(OrthoState)	
	SME_ON_EVENT(EXT_EVENT_ID_POWER, OnPowerDownEXT_EVENT_ID_POWER, Join1) // Test Case: Exit an orthogonal state
	SME_ON_INTERNAL_TRAN(EXT_EVENT_ID_PAUSE_RESUME, OnOrthoInternal)
SME_END_STATE_DEF
...
int OnOrthoInternal(SME_APP_T *pApp, SME_EVENT_T *pEvent)
{
	SME_APP_T *pRegionAppList[4];
	SME_THREAD_CONTEXT_T *pThreadContextList[4];

	printf("OnOrthoInternal\n");

	SmeGetRegions(&SME_GET_APP_VAR(Player1),4,pRegionAppList, pThreadContextList);
	return 0;
}
