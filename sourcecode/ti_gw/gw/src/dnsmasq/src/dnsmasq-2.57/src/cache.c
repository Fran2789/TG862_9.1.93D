...
void cache_init(void)
{
  ...
  if (option_bool(OPT_LOG))
    addrbuff = safe_malloc(INET6_ADDRSTRLEN);  //UNIHAN MOD
...
void dump_cache(time_t now)
{
  ...
  if (!addrbuff && !(addrbuff = whine_malloc(INET6_ADDRSTRLEN))) //UNIHAN MOD
    return;
	...
	for (cache = hash_table[i]; cache; cache = cache->hash_next)
	  {
		...
		if (cache->flags & F_IPV4)
          ...
          inet_ntop(AF_INET6, &cache->addr.addr, addrbuff, INET6_ADDRSTRLEN); //UNIHAN MOD
	      }
#else
            else 
  ...
  if (addr)
    {
     ...
     //UNIHAN MOD START
      ...
      addr, addrbuff, flags & F_IPV4 ? ADDRSTRLEN:INET6_ADDRSTRLEN);
     //UNIHAN MOD END
#else
