...
// ARRIS ADD - to make ZSI mode runtime instead of compile/config flag
static u32 spi_cmd_read_mask = 0x01 << 21;
static u32 spi_dout_dir_mask = 0x00000000;

void ti_codec_spi_set_zsimode( int zsimode )
{
	if(zsimode == 1) 
	{
		spi_cmd_read_mask = 0x01 << 23;
	}
	printk(KERN_DEBUG "ti_codec_spi_zsimode (%d)\n", zsimode);
}
EXPORT_SYMBOL(ti_codec_spi_set_zsimode);
// END ARRIS ADD - to make ZSI mode runtime instead of compile/config flag
...
no_freq:
	...
	if(spi->mode & SPI_3_WIRE)
		cmd_to_write |= spi_dout_dir_mask;  // ARRIS CHANGE
...
static int ti_codec_spi_transfer(struct spi_device *spi,struct spi_transfer *t)
{
	...
	if( t->tx_buf ){	
		...
		cmd_to_write = spi_cmd_read_mask;  // ARRIS CHANGE
	...
	ti_codec_spi->base->spcr1 = 
		((ti_codec_spi->slave[spi->chip_select].cmd_to_write) | spi_dout_dir_mask);  // ARRIS CHANGE
