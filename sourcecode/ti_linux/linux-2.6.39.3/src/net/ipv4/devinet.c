...
/* ARRIS ADD START */
/* function used to retrieve information only, it does not wait on the rtnl semaphore.  Function returns if semaphore is not available. */
int devinet_ioctl_non_blocking(struct net *net, unsigned int cmd, void __user *arg)
{
	struct ifreq ifr;
	struct sockaddr_in sin_orig;
	struct sockaddr_in *sin = (struct sockaddr_in *)&ifr.ifr_addr;
