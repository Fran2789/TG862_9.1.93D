...
#define UDHCP_ERROR_TEXT_SIZE 30
int read_interface(char *interface, int *ifindex, u_int32_t *addr, unsigned char *arp)
{
	int fd;
	struct ifreq ifr;
	struct sockaddr_in *sin;
	char buf[UDHCP_ERROR_TEXT_SIZE];
	...
	if((fd = socket(AF_INET, SOCK_RAW, IPPROTO_RAW)) >= 0) {
		...
		if (addr) { 
			if (ioctl(fd, SIOCGIFADDR, &ifr) == 0) {
				...
				*addr = sin->sin_addr.s_addr;
				...
				strerror_r(errno,buf,UDHCP_ERROR_TEXT_SIZE);
				LOG(LOG_ERR, "SIOCGIFADDR failed!: %s", buf);
				return -1;
			}
		}
		...
		if (ioctl(fd, SIOCGIFINDEX, &ifr) == 0) {
			...
			*ifindex = ifr.ifr_ifindex;
			...
			strerror_r(errno,buf,UDHCP_ERROR_TEXT_SIZE);
			LOG(LOG_ERR, "SIOCGIFINDEX failed!: %s",buf);
			return -1;
		}
		if (ioctl(fd, SIOCGIFHWADDR, &ifr) == 0) {
			...
			strerror_r(errno,buf,UDHCP_ERROR_TEXT_SIZE);
			LOG(LOG_ERR, "SIOCGIFHWADDR failed!: %s", buf);
			return -1;
		}
	} else {
		strerror_r(errno,buf,UDHCP_ERROR_TEXT_SIZE);
		LOG(LOG_ERR, "socket failed!: %s",buf);
		return -1;
	}
	close(fd);
	return 0;
...
int listen_socket(unsigned int ip, int port, char *inf)
{
	...
	if ((fd = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0) {
		strerror_r(errno,buf,UDHCP_ERROR_TEXT_SIZE);
		DEBUG(LOG_ERR, "socket call failed: %s", buf);
		return -1;
	}
...
int raw_socket(int ifindex)
{
	...
	if (bind(fd, (struct sockaddr *) &sock, sizeof(sock)) < 0) {
		strerror_r(errno,buf,UDHCP_ERROR_TEXT_SIZE);
		DEBUG(LOG_ERR, "bind call failed: %s", buf);
		close(fd);
		return -1;
	}
