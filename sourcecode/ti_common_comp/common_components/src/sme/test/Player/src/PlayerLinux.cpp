...
unsigned long ConsoleProc(void *Param);
...
int main(int argc, char* argv[])
{
	...
	int ret;
	printf("Cross-Platform Player Applicaton. IntelliWizard Inc.\n");
	...
	SmeSetExtEventOprProc(XGetExtEvent, XDelExtEvent,XPostExtThreadEvent);
	...
	ret = XCreateThread((void *( *)(void *))ConsoleProc, NULL, &ThreadHandle);
...
unsigned long ConsoleProc(void *Param)
{
	...
	while(TRUE)
	{
		...
		switch(nParam1)
		{
			...
			case 'x':
			case 'X'://Quit
				XPostExtIntEvent(SME_EVENT_EXIT_LOOP, 0, 0, NULL,0,SME_EVENT_CAT_OTHER);
			...
			case '?'://Help
				...
				break;
			case '1':
				XPostExtIntEvent(EXT_EVENT_ID_POWER, 0, 0, NULL,0,SME_EVENT_CAT_OTHER);
				printf("Post a message: EXT_EVENT_ID_POWER!\n");
				break;
			case '2':
				XPostExtIntEvent(EXT_EVENT_ID_PAUSE_RESUME, 0, 0, NULL,0,SME_EVENT_CAT_OTHER);
