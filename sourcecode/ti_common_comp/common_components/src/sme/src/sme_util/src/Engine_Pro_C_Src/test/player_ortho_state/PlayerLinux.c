...
{
	XTHREADHANDLE ConsoleThreadHandle = 0;
	XTHREADHANDLE AppThread2Handle = 0;
	int ret;
	...
	ret = XCreateThread(ConsoleProc, NULL, &ConsoleThreadHandle);

	// Create the Player2 application thread 
	ret = XCreateThread(AppThread2Proc, NULL, &AppThread2Handle);
	...
	XWaitForThread(ConsoleThreadHandle);
	XCLOSE_HANDLE(ConsoleThreadHandle);
	XWaitForThread(AppThread2Handle);
	XCLOSE_HANDLE(AppThread2Handle);
...
#endif
{
	...
	SmeActivateApp(&SME_GET_APP_VAR(Player2),NULL);
	SmeRun();
