...
/* ARRIS MOD BEGIN : Allow -h thread support */
enum {
	OPT_d = (1 << 0),
	OPT_n = (1 << 1),
	OPT_b = (1 << 2),
...
int top_main(int argc UNUSED_PARAM, char **argv)
{
	...
	col = getopt32(argv, "d:n:b"IF_FEATURE_SHOW_THREADS("h")IF_FEATURE_TOPMEM("m"), &str_interval, &str_iterations);    // ARRIS MOD
#if ENABLE_FEATURE_TOPMEM
    ...
    /* ARRIS ADD BEGIN - Handle show threads */
#if ENABLE_FEATURE_SHOW_THREADS
    if (col & OPT_h) {
        scan_mask |= PSSCAN_TASKS;
    }
