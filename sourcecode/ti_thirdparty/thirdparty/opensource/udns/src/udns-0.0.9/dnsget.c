...
printrr(const struct dns_parse *p, struct dns_rr *rr) {
  ...
  if (verbose > 0) {
    if (verbose > 1) {
      ...
      printf("%s. %s %u ", dns_dntosp(rr->dnsrr_dn), dns_typename(rr->dnsrr_typ), rr->dnsrr_ttl);
  }
...
/* ARRIS ADD START */
int HasIPv4Address(char *ifname)    
{
    struct ifreq ifr;
    ...
    if (ioctl(fd, SIOCGIFADDR, &ifr) != 0) {
        close(fd);
        return 0; /* can't find ipv4 address */
    }

    return 1;/* find ipv4 address */
}
#define LINE_LENGTH 80
int HasIPv6Address(char *ifname)    
{
	FILE *fp;
    ...
    if (fp == NULL)
    {
        printf("can't open file if_inet6\n");
        return 0;
    }
    ...
    while (fgets(line, LINE_LENGTH, fp) != NULL) 
    {
        ...
        if ((memcmp(if_name, interface,strlen(interface))==0) && (scope[0] == '0'))
            return 1; /* find global ipv6 address on erouter0 */
    }
...
int main(int argc, char **argv) {
  ...
  /* ARRIS ADD START, default value is ipv4 */
  int ipv4_only = 1;
  /* ARRIS ADD END */
  ...
  /* ARRIS ADD START */
  if(1 == HasIPv4Address(interface))
  {
    ipv4_only = 1;
  }
  else if(1 == HasIPv6Address(interface))
  {
    ipv4_only = 0;
  }
  else
    printf("no legal ip address on erouter0 !!!\n");
  /* ARRIS ADD END */
  if (dns_init(NULL, 0, ipv4_only) < 0 || !(nctx = dns_new(NULL))) /* ARRIS MOD  */
