...
struct sk_buff {
    ...
    __u16               vpid_vlan_tci;   // Need to save the vpid vlan tci before it will erased - We will use it in the ingress_hook.
#endif  /* CONFIG_TI_PACKET_PROCESSOR */
	
	/* ARRIS ADD BEGIN : mark packets that pass socket filter check in docsis bridge bypass */
	unsigned char       filter_match;
	/* ARRIS ADD END */
	...
	atomic_t		users;
#ifdef INCLUDE_SERCOMM_GW //reserved for eGW 
	union{
		char                    egw[64];
	}sc;
#endif
