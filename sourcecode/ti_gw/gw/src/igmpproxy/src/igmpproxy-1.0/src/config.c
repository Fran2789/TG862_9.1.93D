...
loadConfig (char *configFile)
{
    ...
    {
        log_msg (LOG_ERR, 0, PROGNAME ":Unable to open config file  %s",
                 configFile);
    }
    ...
    /* ARRIS MOD END */
    {
        ...
        if (strcasecmp ("phyint", token) == 0)
        {
            ...
            if (rc == 0)
            {
                ...
                closeConfigFile ();
                log_msg (LOG_ERR, 0,
                         PROGNAME ":Unknown token '%s' in configfile", token);
                return 0;
            }
            else
            {
                applydefaults (v);	// compliment configuration values by defaults
                ...
                if ( v->uv_ifindex == 0)
                {
                    log_msg (LOG_ERR, errno,
                             PROGNAME ":Fatal: Kernel can not make index from ifname =%s",
                             v->mvif_name);
                    return -1;
                }
...
applydefaults (struct mvif *v)
{
    v->uv_mld_query_rsp_interval =
        (v->uv_mld_query_rsp_interval >
         0) ? v->uv_mld_query_rsp_interval : MCAST_DEFAULT_QUERY_RESPONSE_INTERVAL;
    v->uv_mld_query_interval =
        (v->uv_mld_query_interval >
         0) ? v->uv_mld_query_interval : MCAST_DEFAULT_QUERY_INTERVAL;
    v->uv_mld_robustness =
        (v->uv_mld_robustness >
         0) ? v->uv_mld_robustness : MCAST_DEFAULT_ROBUSTNESS;
    v->uv_mld_version =
        (v->uv_mld_version > 0) ? v->uv_mld_version : MCAST_DEFAULT_VERSION;
    v->uv_older_host_present = v->uv_mld_version ;
    v->uv_mld_llqi = (v->uv_mld_llqi > 0) ? v->uv_mld_llqi : MCAST_DEFAULT_LAST_LISTENER_QUERY_INTERVAL ;
    v->uv_fastleave = (v->uv_fastleave > 0) ? v->uv_fastleave : 0;
    v->uv_flags = (v->uv_flags > 0) ? v->uv_flags : VIFF_DOWNSTREAM;
    v->uv_startupQueryCount =
        (v->uv_startupQueryCount >
         0) ? v->uv_startupQueryCount : MCAST_DEFAULT_ROBUSTNESS;
    v->uv_startupQueryInterval =
        (v->uv_startupQueryInterval >
         0) ? v->uv_startupQueryInterval : MCAST_DEFAULT_QUERY_INTERVAL;
}
...
parsePhyintToken (int countInfces, struct mvif *v)
{
    ...
    {
        log_msg (LOG_ERR, 0, PROGNAME ":Error: IF name too long  interface %s.",
                 token);
        return 0;
    }
    IF_DEBUG (DEBUG_IF)
    log_msg (LOG_DEBUG, 0, PROGNAME ":Config: IF: Config for interface %s.",
             token);
    ...
    while (token != NULL)
    {
        ...
        else if (strcasecmp ("protocolV", token) == 0)
        {
            ...
            {
                log_msg (LOG_WARNING, 0,
                         PROGNAME ":protocolV must 1 to 3 , not  %d", n);
                parseError = 1;
                break;
            }
            if ( n==1 )
                v->uv_mld_version = IGMPv1;
            else if ( n==2)
                v->uv_mld_version = IGMPv2;
            else
                v->uv_mld_version = IGMPv3;
	    v->uv_older_host_present = v->uv_mld_version ;
        }
        else if (strcasecmp ("fastleave", token) == 0)
        {
        ...
        else if (strcasecmp ("llqi", token) == 0)
        {
            ...
            IF_DEBUG (DEBUG_IF)
            log_msg (LOG_DEBUG, 0,
                     PROGNAME ":Config: IF %s: Got llqi token '%s'.",
                     v->mvif_name, token);
            n = atoi (token);
            if (n < 1)
            {
                log_msg (LOG_WARNING, 0,
                         PROGNAME ":llqi must be 1 sec or more and not %d",
                         n);
                parseError = 1;
                break;
            }
        ...
        else if (strcasecmp ("llqc", token) == 0)
        {
            ...
            IF_DEBUG (DEBUG_IF)
            log_msg (LOG_DEBUG, 0,
                     PROGNAME ":Config: IF %s: Got llqc token '%s'.",
                     v->mvif_name, token);
            n = atoi (token);
            ...
            {
                log_msg (LOG_WARNING, 0,
                         PROGNAME ":llqc must be  greater (or eq) then 1 not %d",
                         n);
                parseError = 1;
                break;
            }
        ...
        else if (strcasecmp ("startupQueryCount", token) == 0)
        {
            ...
            {
                log_msg (LOG_WARNING, 0,
                         PROGNAME ":startupQueryCount must be  greater (or eq) then 1 not %d",
                         n);
                parseError = 1;
                break;
            }
        ...
        else if (strcasecmp ("queryResponseInterval", token) == 0)
        {
            ...
            IF_DEBUG (DEBUG_IF)
            log_msg (LOG_DEBUG, 0,
                     PROGNAME ":Config: IF %s: Got queryResponseInterval token '%s'.",
                     v->mvif_name, token);
            n = atoi (token);
            ...
            {
                log_msg (LOG_WARNING, 0,
                         "queryResponseInterval must be a positive value , not %d",
                         n);
                parseError = 1;
                break;
            }
