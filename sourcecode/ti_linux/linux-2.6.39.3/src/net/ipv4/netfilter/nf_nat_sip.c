...
//Arris add
    if(mta0_dev == NULL)
    {
        mta0_dev = dev_get_by_name(&init_net, IFNAME_MTA_0 );
    }
...
static unsigned int ip_not_nat_via(struct sk_buff *skb, unsigned int dataoff,
				   const char **dptr, unsigned int *datalen)
{
	/* if we are here then proto is udp and its reply
	   this function is called in case if forced_dport is detected.
	   in case of NAT the same case handled by ip_nat_sip func above.
	*/
	enum ip_conntrack_info ctinfo;
	struct nf_conn *ct = nf_ct_get(skb, &ctinfo);
	struct nf_conn_help *help = nfct_help(ct);
...
//add end
	...
	/* next code duplicates code above. it may be moved to inline func */
	if (!skb_make_writable(skb, skb->len))
		return NF_DROP;
...
static void __exit nf_nat_sip_fini(void)
{
	...
	rcu_assign_pointer(nf_not_nat_sip_via_hook, NULL);
...
static int __init nf_nat_sip_init(void)
{
	...
	BUG_ON(nf_not_nat_sip_via_hook != NULL);
	...
	rcu_assign_pointer(nf_not_nat_sip_via_hook, ip_not_nat_via);
