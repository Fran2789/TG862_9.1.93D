...
#include <linux/workqueue.h>    /* ARRIS ADD */
...
/* ARRIS ADD BEGIN : Use a WorkQueue to handle commands that need to be handled in process state,
 *                   but are issued in interrupt / softIRQ state.
 *                   Add pretty much anything here that needs to sleep for large amounts of time
 */
#define SWT_RESET           (0)
#define SWT_BOUNCE_LINK     (1)

static struct workqueue_struct *switchWorkQueue = NULL;

typedef struct {
    struct work_struct switchWork;

    /* We can add more commands here if we see other places where Intel tries to issue
     * commands that need to sleep in non-process oriented state.
     * For now, only reset seems to be issued in interrupt state 
     */
    unsigned int cmd;
} switchWork_t;

static void switchWorkTask( struct work_struct* work)
{
    switchWork_t *switchWork = (switchWork_t*)work;

    switch( switchWork->cmd ) 
    {
        case SWT_RESET:
            /* Into reset */
            ...
            /* Reprogram default VLANS */
            pCslSwitchClbks->cslSwitchStart();
            break;

        case SWT_BOUNCE_LINK:
            printk(KERN_ERR "PSADRO: In Work Queue - call callback\n");

            pCslSwitchClbks->cslArrisBounceLink();
            break;

        default:
            printk(KERN_ERR "\nSWITCH --> Unknown WorkQueue Command %d\n", switchWork->cmd);
            break;
    }
...
PAL_Result switch_init(PHY_DEVICE * pSetSwitchInfo)
{
    ...
    /* ARRIS ADD BEGIN : Initialize a workqueue for tasks that need to sleep */
    switchWorkQueue = create_workqueue("SwitchWorkQueue");
    /* ARRIS ADD END */
    ...
    if(
// ARRIS DELETE BEGIN : This API doesn't play nice with realtek switches
...
PAL_Result switch_reset(int init)
{
    ...
    if ( -1 != ext_switch_reset_gpio )
    {
        ...
        /* ARRIS MOD BEGIN : Schedule the reset in kernel thread, so it can sleep */
        /* If not Init, schedule the work... */
        if (!init)
        {
            // queue the work to a context where we can sleep....
            if (switchWorkQueue) 
            {
                switchWork_t *work = (switchWork_t*)kmalloc(sizeof(switchWork_t), GFP_ATOMIC);
                if (work) 
                {
                    INIT_WORK( (struct work_struct*)work, switchWorkTask );
                    work->cmd = SWT_RESET;

                    queue_work( switchWorkQueue, (struct work_struct*)work );
                }
            }
        }
        /* ARRIS MOD END */
...
{
    // ARRIS MOD START
    PAL_Result retVal = PAL_False;

    if (pCslSwitchClbks->cslRegGet)
    {
        retVal = pCslSwitchClbks->cslRegGet( 0, addr, data, 4 );
    }
...
PAL_Result switch_dbgRegWrite( Uint32 addr, Uint32 * data )
{
    ...
    {
        retVal = pCslSwitchClbks->cslRegSet( 0, addr, data, 4 );
    }
...
// ARRIS MOD BEGIN : Don't call MDIO directly - extend the lower level API
// maybe speak a bit with Intel on this - it's a strange layering here...
#if 0
...
{
    Int32 retVal = PAL_False;

    RESULT_ASSERT(PortNum < 6);

    /* PORT_STATUS enum Start from 1 - the register values start from 0 */
    if (pCslSwitchClbks->cslPortStatusSet)
    {
        retVal = pCslSwitchClbks->cslPortStatusSet( PortNum-1, status );
    }
...
{
    PORT_STATUS portStatus = PORT_FORWARDING;

    RESULT_ASSERT(PortNum < 6);

    /* PORT_STATUS enum Start from 1 - the register values start from 0 */
    if (pCslSwitchClbks->cslPortStatusGet)
    {
        portStatus = (PORT_STATUS)pCslSwitchClbks->cslPortStatusGet(PortNum-1);
    }
...
{
    PORT_LINK portLink = PORT_LINK_DOWN;

    RESULT_ASSERT(PortNum < 6);

    if (pCslSwitchClbks->cslPortLinkGet)
    {
        portLink = (PORT_LINK)pCslSwitchClbks->cslPortLinkGet(PortNum-1);
    }
    ...
    )
{
    ...
    if (pCslSwitchClbks->cslPortFCSet)
    {
        retVal = pCslSwitchClbks->cslPortFCSet(PortNum-1, FCStatus);
    }
...
{
    PORT_FC portFC = PORT_FC_AUTO;

    RESULT_ASSERT(PortNum < 6);

    if (pCslSwitchClbks->cslPortFCGet)
    {
        portFC = (PORT_FC)pCslSwitchClbks->cslPortFCGet(PortNum-1);
    }
    ...
    )
{
    ...
    if (pCslSwitchClbks->cslPortDuplexSet)
    {
        retVal = pCslSwitchClbks->cslPortDuplexSet(PortNum-1, duplexStatus);
    }
...
{
    PORT_DUPLEX portDuplex = PORT_DUPLEX_FULL;

    RESULT_ASSERT(PortNum < 6);

    if (pCslSwitchClbks->cslPortDuplexGet)
    {
        portDuplex = (PORT_DUPLEX)pCslSwitchClbks->cslPortDuplexGet(PortNum-1);
    }
    ...
    )
{
    ...
    if (pCslSwitchClbks->cslPortSpeedSet)
    {
        pCslSwitchClbks->cslPortSpeedSet(PortNum-1, speedStatus);
        retVal = PAL_True;
    }
...
{
    PORT_SPEED portSpeed = PORT_SPEED_1000;

    RESULT_ASSERT(PortNum < 6);

    if (pCslSwitchClbks->cslPortSpeedGet)
    {
        portSpeed = (Int32)pCslSwitchClbks->cslPortSpeedGet(PortNum-1);
    }
    ...
    )
{
    ...
    if (pCslSwitchClbks->cslPortAutoSet)
    {
        retVal = pCslSwitchClbks->cslPortAutoSet(PortNum-1, (Int32)autoStatus);
    }
...
{
    PORT_AUTO portAuto = PORT_AUTO_NO;

    RESULT_ASSERT(PortNum < 6);

    if (pCslSwitchClbks->cslPortAutoGet)
    {
        portAuto = (PORT_AUTO)pCslSwitchClbks->cslPortAutoGet(PortNum-1);
    }
    ...
    )
{
    ...
    if ( pCslSwitchClbks->cslPortResetStats )
    {
        retVal = pCslSwitchClbks->cslPortResetStats(PortNum-1);
    }
...
// ARRIS ADD : Dump All Registers
Int32 switch_dumpAllRegisters( void )
{
    Int32 retVal = PAL_False;

    if (pCslSwitchClbks->cslDumpAllRegisters)
    {
        retVal = pCslSwitchClbks->cslDumpAllRegisters();
    }
...
// ARRIS delete : very specific to the atheros, I think...
#if 0
    RESULT_ASSERT(elem != NULL);
    ...
    {
        pCslSwitchClbks->vlan_member_add(0,vlnId, portNum, tag);    // ARRIS MOD
    }

#if 0 // ARRIS DELETE : Atheros specific
...
/* ARRIS ADD BEGIN : Switch API extensions */
PAL_Result switch_arrisGetPortStatus(Uint32 PortNum, PSW_PORT_STATUS portStatus)
{
    PAL_Result retVal = PAL_False;

    RESULT_ASSERT(PortNum < 6);

    if(pCslSwitchClbks->cslArrisGetPortStatus != NULL)
    {
        if (pCslSwitchClbks->cslArrisGetPortStatus(PortNum-1,portStatus) == PAL_SOK)
        {
            retVal = PAL_True;
        }
    }
...
PAL_Result switch_arrisGetPortStats(Uint32 PortNum, Uint64* txPkts, Uint64* rxPkts)
{
    PAL_Result retVal = PAL_False;

    RESULT_ASSERT(PortNum < 6);

    if(pCslSwitchClbks->cslArrisGetPortStats != NULL)
    {
        if (pCslSwitchClbks->cslArrisGetPortStats(PortNum-1,txPkts,rxPkts) == PAL_SOK)
        {
            retVal = PAL_True;
        }
    }
...
PAL_Result switch_arrisSetCertConfig()
{
    PAL_Result retVal = PAL_False;

    if(pCslSwitchClbks->cslArrisSetCertConfig != NULL)
    {
        if (pCslSwitchClbks->cslArrisSetCertConfig() == PAL_SOK)
        {
            retVal = PAL_True;
        }
    }
...
PAL_Result switch_arrisBounceLink()
{
    PAL_Result retVal = PAL_False;

    // Make sure this runs in process mode, since we sleep for 3 seconds...
    if(pCslSwitchClbks->cslArrisBounceLink != NULL)
    {
        // queue the work to a context where we can sleep....
        if (switchWorkQueue) 
        {
