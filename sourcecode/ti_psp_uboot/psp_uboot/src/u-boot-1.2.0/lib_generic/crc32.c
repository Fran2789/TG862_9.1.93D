    ...
    uInt len;
{
...
#ifdef CONFIG_EXTENDED_FLASH
        int full_block = 1;
        switch_res = process_flash_bank(NULL, buf, SFI_BUF_SIZE, &retlen, 0);
        if (switch_res == EMM_HIGH)
        {
            buf -= FLASH_BORDER_OFFSET_16MB;
        }
        else if (switch_res == EMM_LOW)
        {
            if (retlen != SFI_BUF_SIZE)
            {
               full_block = 0;
            }
        }
      /* Use this only if the address is 4-byte aligned and occupies one bank low or high*/
    if ((!((unsigned)buf & 3)) && full_block) {
#else
    ...
    if (len) do {
#ifdef CONFIG_EXTENDED_FLASH
    switch_res = process_flash_bank(NULL, buf, 1, &retlen, 0);
    if (switch_res == EMM_HIGH)
    {
        buf -= FLASH_BORDER_OFFSET_16MB;
        DO1(buf);
    ...
    else if ((switch_res == EMM_LOW) || (switch_res == EMM_NOT_FL))
    {
        DO1(buf);
    }
#else
