...
/* ARRIS CHANGE START */
/* ti_register_selective_packet_handler & ti_unregister_selective_packet_handler functions
   add missing dev_put() calls */
...
int ti_register_selective_packet_handler (char *br_name, int (*packet_handler)(struct sk_buff *skb), int priority)
{
    ...
    printk (KERN_DEBUG "DEBUG: Register selective packet handler successful %s\n",br_name); /* ARRIS CHANGE */
    dev_put(dev);
    return 0;
}
