...
#include <string.h>
#include <string>
#include <fstream>      // UNIHAN ADD , PROD00199235 PROD00197285
#include "Portable.h"
#include "OptFQDN.h"
#include "Logger.h"
#include "Portable.h"
...
#define CLIENT_ADDR_HOSTANME_FILE "/nvram/8/dhcpd/dhcpv6_host"
#define CLIENT_ADDR_HOSTANME_DEL  "/nvram/8/dhcpd/dhcpv6_host_del"
...
// UNIHAN ADD START , PROD00199235 PROD00197285
    fstream client_host;
    string  stream_line;
    string  t_string( fqdn_addr );
    string  a_string;
    size_t found = false;
    size_t del_flag = false;

    client_host.open( CLIENT_ADDR_HOSTANME_FILE , ios::out | ios::in | ios::app );
    if( client_host )
    {
        while( getline( client_host , stream_line ) )
        {
            found = stream_line.find( t_string );
        ...
        if( del_flag == true )
        {
            if( delete_line( t_string ) == false ) 
            {
                Log(Debug) << "Remove line of file failed" << LogEnd;
            }
        }
        
        a_string = t_string + " " + fqdn_;
        if( !client_host.is_open() )
        {
            client_host.open( CLIENT_ADDR_HOSTANME_FILE , ios::out | ios::in | ios::app );
            if( client_host.is_open() )
            {
                client_host << a_string << endl;
                client_host.close();
            }
            else
            {
                Log(Debug) << "Can't Open File : " << CLIENT_ADDR_HOSTANME_FILE << "for add hostname"<< LogEnd;
            }
        }
    }
    else
    {
        Log(Debug) <<  "Can't OPEN : " << CLIENT_ADDR_HOSTANME_FILE << LogEnd ;
    }
// UNIHAN ADD END , PROD00199235 PROD00197285
...
// UNIHAN ADD START , PROD00199235 PROD00197285
bool TOptFQDN::delete_line( const string &del_str )
{
    string  stream_line;
    size_t found = false;

    ifstream client_host( CLIENT_ADDR_HOSTANME_FILE );
    if( !client_host.is_open() )
    {
        Log(Debug) << "OPEN FAILED : " << CLIENT_ADDR_HOSTANME_FILE << LogEnd;
        return false;
    }

    ofstream client_tmp(CLIENT_ADDR_HOSTANME_DEL);
    if( !client_tmp.is_open() )
    {
        Log(Debug) << "OPEN FAILED : " << CLIENT_ADDR_HOSTANME_DEL << LogEnd;
        client_host.close();
        return false;
    }
    ...
    while( getline( client_host , stream_line ) )
    {
        found = stream_line.find( del_str );
        if( found == string::npos )
        {
           client_tmp << stream_line << endl; 
        }
    }
    ...
    if( remove( CLIENT_ADDR_HOSTANME_FILE ) != 0 )
    {
        perror( "Error remove file" );
    }

    if( rename( CLIENT_ADDR_HOSTANME_DEL , CLIENT_ADDR_HOSTANME_FILE ) != 0 )
    {
