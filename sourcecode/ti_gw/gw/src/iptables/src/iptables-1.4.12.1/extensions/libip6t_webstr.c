...
static void 
parse_string(const char *s, struct ip6t_webstr_info *info) 
{        
        if (strlen(s) <= BM6_MAX_NLEN) strcpy(info->string, s); 
...
static struct xtables_match webstr6 = {  
        .name           = "webstr", 
...
void _init(void) 
{ 
        xtables_register_match(&webstr6);
} 
