...
void reply_query(int fd, int family, time_t now)
{
  ...
  if ((RCODE(header) == SERVFAIL || RCODE(header) == REFUSED || RCODE(header) == NXDOMAIN) &&   /* UNIHAN MOD */
	  ...
	  if ((nn = resize_packet(header, (size_t)n, pheader, plen)))
	    {
          ...
          //int out=if_nametoindex("erouter0");
  ...
  if (forward->forwardall == 0 || --forward->forwardall == 1 || 
      (RCODE(header) != REFUSED && RCODE(header) != SERVFAIL && RCODE(header) != NXDOMAIN))   /* UNIHAN MOD */
    {
