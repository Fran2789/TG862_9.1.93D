/* dnsmasq is Copyright (c) 2000-2011 Simon Kelley
   ...
   #   12-Jun-2011: Inserted dns-upstream option
   #   1-Dec-2011: open ipv6 socket to specific address instead of any address
   #
...
#if PUMA5_SOC_TYPE
  /*ARRIS ADD START for PROD00216813 */
  /* in puma5 version,eth0 have link local address, it results DNSmasq always bind socket on eth0, not eth0.2. 
       So skip eth0 address */
...
int reload_servers(char *fname)
{
  ...
  while ((line = fgets(daemon->namebuff, MAXDNAME, f)))
      ...
      else if (inet_pton(AF_INET6, token, &addr.in6.sin6_addr) > 0)
	{
      ...
      if ((!(token = strtok(NULL, " \t\n\r"))) || 
          (inet_pton(AF_INET6, token, &source_addr.in6.sin6_addr) <= 0))
      {
	...
	continue;
	 ...
	 it gets reversed again in check_servers */
      ...
      serv->interface[0] = 0;
	  if (daemon->dnsout) {
		  strncpy(&serv->interface[0], daemon->dnsout, MAXINTERFACENAME);
	  }
      serv->sfd = NULL;
