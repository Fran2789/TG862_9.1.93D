...
{
    // ARRIS ADD START
    // Force dhcpv4/v6 to send release
    // Codes here will cover all system reboots.
    char cmd[256];
	sprintf(cmd, "ps|grep \'ti_udhcpc -i erouter0\' |grep -v grep | cut -c 1-5 > /var/tmp/releasev4");
    system(cmd);
	system("kill -sigusr2 `cat /var/tmp/releasev4`");
	sprintf(cmd, "ps|grep \'ti_dhcp6c -i erouter0\' |grep -v grep | cut -c 1-5 > /var/tmp/releasev6");
    system(cmd);
    system("kill -sigusr2 `cat /var/tmp/releasev6`");
    sprintf(cmd, "ps|grep \'ti_syslogd\' |grep -v grep | cut -c 1-5 > /var/tmp/syslogd.pid");
    system(cmd);
    system("kill -sigusr2 `cat /var/tmp/syslogd.pid`");
    // ARRIS ADD END
...
int init_main(int argc UNUSED_PARAM, char **argv)
{
	...
	putenv((char *) "HOME=/");
// ARRIS ADD START
#ifdef CONFIG_TI_DMALLOC_UTIL
	putenv((char *) "DMALLOC_OPTIONS=debug=0x000002,inter=100,log=/var/tmp/dmalloclog.%d");
#endif
// ARRIS ADD END
