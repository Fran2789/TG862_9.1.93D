...
static irqreturn_t hw_mutex_isr(int irq, void *dev_id)
{
	...
	switch(pmaster->mode){
		case MUTEX_FIFO_SCHE:	
			hmutex_ops->clr_intr(pmaster);                                      //ARRIS MOD add HW MUTEX patch IPS 6000014921
			ret = IRQ_HANDLED;                                                  //ARRIS MOD add HW MUTEX patch IPS 6000014921
			for (i = 0; i< HW_MUTEX_TOTAL; i++) {
				hmutex = &pmaster->hw_mutexes[i]; 
				spin_lock(&hmutex->irq_lock);                                   //ARRIS MOD add HW MUTEX patch IPS 6000014921
				if (HW_MUTEX_REQUESTING == atomic_read(&hmutex->status)){ 
					...
					}                                                           //ARRIS MOD add HW MUTEX patch IPS 6000014921
				}                                                               //ARRIS MOD add HW MUTEX patch IPS 6000014921
			spin_unlock(&hmutex->irq_lock);                                     //ARRIS MOD add HW MUTEX patch IPS 6000014921
			}
			break;
...
void hw_mutex_lock(uint8_t mutex)
{
	...
	spin_lock_irqsave(&hmutex->irq_lock,flags);	                //ARRIS MOD add HW MUTEX patch IPS 6000014921
	hw_mutex_set_owner(hmutex);                                 //ARRIS MOD add HW MUTEX patch IPS 6000014921
	if (hmutex_ops->lock(hmutex,0)) {
	        spin_unlock_irqrestore(&hmutex->irq_lock,flags);    //ARRIS MOD add HW MUTEX patch IPS 6000014921	
		return ;
	}                                                           //ARRIS MOD add HW MUTEX patch IPS 6000014921
...
long __sched hw_mutex_lock_interruptible(uint8_t mutex)
{
	...
	if (hmutex_ops->lock(hmutex,0)) {
		spin_unlock_irqrestore(&hmutex->irq_lock,flags);	    //ARRIS MOD add HW MUTEX patch IPS 6000014921
		return 0;                                               //ARRIS MOD add HW MUTEX patch IPS 6000014921
	}                                                           //ARRIS MOD add HW MUTEX patch IPS 6000014921
...
void hw_mutex_unlock(uint8_t mutex)
{
	struct hw_mutex  * hmutex = NULL;	
	struct hw_mutex_operations *hmutex_ops = to_hw_mutex_ops();
	unsigned long flags;                                            //ARRIS MOD add HW MUTEX patch IPS 6000014921
