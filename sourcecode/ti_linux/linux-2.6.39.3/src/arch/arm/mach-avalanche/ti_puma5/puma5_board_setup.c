...
// ARRIS ADD - Declare a kernel-side Arris Certification flag to be used for dynamic decision making
// regarding code that could impact a DOCSIS cert wave.  FALSE means load is not for a cert wave.  
// TRUE means it is.
unsigned long ArrisCertFlag = 0;
EXPORT_SYMBOL(ArrisCertFlag);
// END ARRIS ADD
 ...
 **************************************************************/
     ...
     */
// ARRIS leave clk_out0 enabled to support reduction in the number of crystals on the modems
 ...
 **************************************************************/
    ...
    PAL_sysGpioOutBit(0, 1); /* ARRIS send a value of "1" on GPIO 0 to turn on power led*/ 
...
// ARRIS override value for gateways
#ifdef INCLUDE_ARRIS_GW 
        eth_reset_gpio_num = EXTPHY_RESET_ARRIS_GATEWAY_GPIO_NUM;
        /* ARRIS take USB components out of reset */
        PAL_sysGpioOutBit(40, 1);
        PAL_sysGpioOutBit(41, 1);  
        PAL_sysGpioOutBit(44, 1);
#else
        PAL_sysGpioOutBit(50, 0); // power down WIFI chip 
#endif 
