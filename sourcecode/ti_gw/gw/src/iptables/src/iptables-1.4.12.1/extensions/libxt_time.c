...
/*SERCOMM ADD*/
#define WEEKDAYS_NONE "None"
#define WEEKDAYS_NONE_LEN 4
/*SERCOMM ADD END*/
...
static unsigned int time_parse_weekdays(const char *arg)
{
	...
	/*SERCOMM ADD*/
	if(strlen(arg)==WEEKDAYS_NONE_LEN) {
		if(strncmp(WEEKDAYS_NONE, arg, WEEKDAYS_NONE_LEN) == 0) {
			return ret;
		}
	}
	/*SERCOMM ADD END*/
