...
//printk(KERN_DEBUG "[vlad] %s:%d  OID_GEN_XMIT_OK=0x%x\n", __func__, __LINE__, *outbuf);
			retval = 0;
		}
		break;
...
//printk(KERN_DEBUG "[vlad] %s:%d  OID_GEN_RCV_OK=0x%x\n", __func__, __LINE__, *outbuf);
			retval = 0;
		}
		break;
	...
	case OID_GEN_XMIT_ERROR:
		...
		if (rndis_per_dev_params [configNr].stats) {
			*outbuf = cpu_to_le32 (rndis_per_dev_params [configNr]
					.stats->tx_errors);
            if (*outbuf)
            //printk(KERN_DEBUG "[vlad] %s:%d OID_GEN_XMIT_ERROR=0x%x\n", __func__, __LINE__, *outbuf);
			retval = 0;
		}
		break;
	...
	case OID_GEN_RCV_ERROR:
		...
		if (rndis_per_dev_params [configNr].stats) {
			*outbuf = cpu_to_le32 (rndis_per_dev_params [configNr]
					.stats->rx_errors);
            if (*outbuf)
            //printk(KERN_DEBUG "[vlad] %s:%d OID_GEN_RCV_ERROR=0x%x\n", __func__, __LINE__, *outbuf);
			retval = 0;
		}
		break;
	...
	case OID_GEN_RCV_NO_BUFFER:
		...
		if (rndis_per_dev_params [configNr].stats) {
			*outbuf = cpu_to_le32 (rndis_per_dev_params [configNr]
					.stats->rx_dropped);
            //printk(KERN_DEBUG "[vlad] %s:%d OID_GEN_RCV_NO_BUFFER=0x%x\n", __func__, __LINE__, *outbuf);
			retval = 0;
		}
		break;
