...
// ARRIS CHANGE - Include the proper define for printk.  This will allow DG860 to compile when we disable printk
...
#include <arris/arris_mod_api.h>      /* ARRIS ADD - define arris module hooks */
...
typedef struct TI_PPM_MCB
{
    ...
    PPM_HANDLE*             session_nonvoice_free_list;
    PPM_HANDLE*             session_nonvoice_occupied_list;

    PPM_HANDLE*             session_voice_free_list;
    PPM_HANDLE*             session_voice_occupied_list;
...
static void ti_ppm_find_and_delete_drop_sessions (int *null_vpid_handle,unsigned int src_ip,unsigned int dst_ip,
...
static PPM_HANDLE* ti_ppm_get_handle (PPM_HANDLE** free_list, PPM_HANDLE** occupied_list)
{
    ...
    if (ptr_handle != NULL)
    {
        listlib_add ((LIST_NODE**)occupied_list, (LIST_NODE*)ptr_handle);
    }
...
)
{
    ...
    listlib_add ((LIST_NODE**)free_list, (LIST_NODE*)ptr_handle);
...
static int ti_ppm_add_hash_table (int session_handle, unsigned int hash_code)
{
    ...
    listlib_add((LIST_NODE**)&ppm_mcb.session_hash[hash_code], (LIST_NODE*)ptr_session_hash);
...
)
{
    ...
    if (ptr_vpid != NULL)
    {
        ...
        if (ptr_vpid->vpid_status == ACTIVE)
        {
            ...
            listlib_add ((LIST_NODE**)&ptr_vpid->vpid_session_mapping, (LIST_NODE*)ptr_session_vpid);
 ...
 *  <0                         - Error
 **************************************************************************/
int ti_ppm_create_pid (TI_PP_PID* ptr_pid, void * netdevPtr)
{
    ...
    if(!ppm_mcb.pid_database[ptr_pid->pid_handle])
    {
        ...
        if (ti_ppd_create_pid(ptr_pid) == 0)
        {
            ...
            if (ptr_internal_pid != NULL)
            {
                ...
                ti_ppm_event_handler (TI_PPM_PID_CREATED, (unsigned int)ptr_pid->pid_handle, (unsigned int)netdevPtr);
        ...
        else
        {
            ...
            ti_ppm_event_handler (TI_PPM_CREATE_PID_FAILED, (unsigned int)ptr_pid->pid_handle, (unsigned int)netdevPtr);
        }
    }
    ...
    if (ptr_internal_pid != NULL)
    {
        ...
        if (ptr_internal_pid->pid_status == ACTIVE)
        {
            ...
            else
            {
                ...
                listlib_add ((LIST_NODE**)&ptr_internal_pid->vpid_list, (LIST_NODE*)ptr_internal_vpid);
...
// ARRIS ADD -- THIS CHANGE CAME FROM SERCOMM
// these functions are exported in ti_ppm_os.c
#ifndef CONFIG_VENDOR_COMCAST
#ifdef INCLUDE_SERCOMM_GW
int (*sc_ppm_create_session_hook)( TI_PP_SESSION* ptr_session, void *ptr ) = NULL;
int (*sc_modify_pp_info_hook)( TI_PP_SESSION* ptr_session, void *ptr ) = NULL;
 ...
 *  param[in] ptr_session - pointer to the session descriptor
 *  param[in] prt - pointer to an skb
 *  param[in] isTunnel - flag that indicates if this is a tunneled session or not
 *  param[out] isNew - 1=new session created, 0=session already exist
 *
 * RETURNS       :
 *      Session Handle  -   Success
 *      <0              -   Error
 **************************************************************************/
int ti_ppm_create_session (TI_PP_SESSION* ptr_session, void* ptr, int isTunnel, unsigned char isVoice, int *isNew)
{
    PPM_OS_CONTEXT      os_context;
    PPM_HANDLE*         ptr_handle;
    PPM_SESSION*        ptr_internal_session;
...
#ifndef CONFIG_VENDOR_COMCAST
#ifdef INCLUDE_SERCOMM_GW    
    if( sc_modify_pp_info_hook )
    {
        sc_modify_pp_info_hook(ptr_session,ptr);
    }

    if ( sc_ppm_create_session_hook )
    {
        if (sc_ppm_create_session_hook(ptr_session, ptr) < 0)
        {
            printk("sc_ppm_create_session_hook < 0");
            return -1;
        }
    }
#endif // INCLUDE_SERCOMM_GW
#endif // Comcast  
    ARRISMOD_CALL( arris_rip_create_pp_session_hook, ptr_session, ptr );
    // ARRIS ADD END
    ...
    *isNew = 1;
    ...
    /* Initialize the session's free and occupied list according to the PID used */
    {
        if (isVoice)
        {
            session_free_list = &ppm_mcb.session_voice_free_list;
            session_occupied_list = &ppm_mcb.session_voice_occupied_list;
        }

        else
        {
            session_free_list = &ppm_mcb.session_nonvoice_free_list;
            session_occupied_list = &ppm_mcb.session_nonvoice_occupied_list;
        }
    }

    /* Generate a handle for the Session. */
    ptr_handle = ti_ppm_get_handle (session_free_list, session_occupied_list);
    if (ptr_handle == NULL)
    {
        /* Critical Section Stop */
        ppm_mcb.os_table.critical_section_end (&os_context);
    ...
    if (ptr_internal_session == NULL)
    {
        /* Error: No memory available; generate the OOM event. */
        ti_ppm_event_handler (TI_PPM_OUT_OF_MEMORY, 0, 0);
        ti_ppm_release_handle (ptr_handle, session_free_list, session_occupied_list);
        /* Critical Section Stop */
        ppm_mcb.os_table.critical_section_end (&os_context);
        printk("ERROR: PP Operation %s cannot be accomplished due to lack of memory\n", __FUNCTION__);
        return -1;
    ...
    ptr_internal_session->isVoice        = isVoice;
    ...
    if (ptr_internal_session != NULL)
    {
        ...
        if (ti_ppd_delete_session (session_handle) == 0)
        {
            ...
            else
            {
                ...
                /* Initialize the session's free and occupied list according to the PID used */
                if (ptr_internal_session->isVoice)
                {
                    session_free_list = &ppm_mcb.session_voice_free_list;
                    session_occupied_list = &ppm_mcb.session_voice_occupied_list;
                }
                ...
                /* Release the session handle. */
                ti_ppm_release_handle (ptr_internal_session->session_handle, session_free_list, session_occupied_list);
...
unsigned int ti_ppm_register_event_handler (void (*event_handler)(unsigned int event_id, unsigned int param1, unsigned int param2))
{
    ...
    listlib_add ((LIST_NODE **)&ppm_mcb.event_info, (LIST_NODE *)ptr_internal_event_handler);
 ...
 *  param[in/out] ptr_list - pointer to the free list to initialize
 *  param[in] first_element - first element index. Usually 0, but for sessions we have 2 lists - voice(0-7) & non-voice (8-256/2048)
 *  param[in] num_elements - number of elements to put in the list
 *
 * RETURNS       :
 *      0   -   Success
 *      <0  -   Error
 **************************************************************************/
static int ti_ppm_init_handles (LIST_NODE** ptr_list, int first_element, int num_elements)
{
    int           index;
    PPM_HANDLE*   ptr_handle;
    ...
    for (index = 0; index < num_elements; index++)
    {
        ...
        ptr_handle->handle = (unsigned char)(index + first_element);

        /* Add the handle to the free list. */
        listlib_add (ptr_list, (LIST_NODE*)ptr_handle);
    }
...
void ti_ppm_pp_event_handler (unsigned short event_id, unsigned int param1, unsigned int param2)
{
    switch( event_id )
    {
       ...
       case TI_PPD_INTERNAL_EVENT_TDOX_EVALUATION:
       {
           ti_ppm_event_handler(TI_PPD_TDOX_EVALUATION, param1, param2);
           break;
       }
        default:
            break;
...
int ti_ppm_initialize (TI_PPM_OS_FUNC_TABLE* ptr_os_table)
{
    ...
    if (ti_ppm_init_handles ((LIST_NODE**)&ppm_mcb.pid_free_list, 0, TI_PP_MAX_PID) < 0)
    {
    ...
    if (ti_ppm_init_handles ((LIST_NODE**)&ppm_mcb.vpid_free_list, 0, TI_PP_MAX_VPID) < 0)
    {
    ...
    /* Initialize the Voice Session Handles. */
    if (TI_PP_MAX_ACCLERABLE_VOICE_SESSIONS)
    {
        if (ti_ppm_init_handles ((LIST_NODE**)&ppm_mcb.session_voice_free_list, 0, TI_PP_MAX_ACCLERABLE_VOICE_SESSIONS) < 0)
        {
            /* Memory Allocation Failed: Cleanup the PID/VPID handles and any Session handles */
            ti_ppm_deinit_handles ((LIST_NODE**)&ppm_mcb.pid_free_list);
            ti_ppm_deinit_handles ((LIST_NODE**)&ppm_mcb.vpid_free_list);
            ti_ppm_deinit_handles ((LIST_NODE**)&ppm_mcb.session_voice_free_list);
            printk("FATAL ERROR: PP Operation %s cannot be accomplished since ti_ppm_init_handles for sessions operation has failed\n", __FUNCTION__);
            return -1;
        }
    }

    /* Initialize the Non Voice Session Handles. */
    if (ti_ppm_init_handles ((LIST_NODE**)&ppm_mcb.session_nonvoice_free_list, TI_PP_MAX_ACCLERABLE_VOICE_SESSIONS, TI_PP_MAX_ACCLERABLE_NONVOICE_SESSIONS) < 0)
    {
        /* Memory Allocation Failed: Cleanup the PID/VPID handles and any Session handles */
        ti_ppm_deinit_handles ((LIST_NODE**)&ppm_mcb.pid_free_list);
        ti_ppm_deinit_handles ((LIST_NODE**)&ppm_mcb.vpid_free_list);
        ti_ppm_deinit_handles ((LIST_NODE**)&ppm_mcb.session_voice_free_list);
        ti_ppm_deinit_handles ((LIST_NODE**)&ppm_mcb.session_nonvoice_free_list);
        printk("FATAL ERROR: PP Operation %s cannot be accomplished since ti_ppm_init_handles for sessions operation has failed\n", __FUNCTION__);
        return -1;
    }
...
int ti_ppm_set_qos_cluster_max_global_credit(int byteCredit, int cluster_id,int max_global_credit)
{
    if (ppm_mcb.pdsp_status != ACTIVE)
    {
        printk("ERROR: PP Operation %s cannot be accomplished while PP status is %s\n", __FUNCTION__, ppm_mcb.pdsp_status == INACTIVE ? "INACTIVE" : "PPM_PSM");
    ...
    ti_ppd_set_qos_cluster_max_global_credit(byteCredit, cluster_id, max_global_credit);

    return 0;
}

int ti_ppm_set_qos_queue_iteration_credit(int byteCredit, int queue_id,int it_credit)
{
    if (ppm_mcb.pdsp_status != ACTIVE)
    {
        printk("ERROR: PP Operation %s cannot be accomplished while PP status is %s\n", __FUNCTION__, ppm_mcb.pdsp_status == INACTIVE ? "INACTIVE" : "PPM_PSM");
    ...
    ti_ppd_set_qos_queue_iteration_credit(byteCredit, queue_id, it_credit);

    return 0;
}

int ti_ppm_set_qos_queue_max_credit(int byteCredit, int queue_id,int max_credit)
{
    if (ppm_mcb.pdsp_status != ACTIVE)
    {
        printk("ERROR: PP Operation %s cannot be accomplished while PP status is %s\n", __FUNCTION__, ppm_mcb.pdsp_status == INACTIVE ? "INACTIVE" : "PPM_PSM");
    ...
    ti_ppd_set_qos_queue_max_credit(byteCredit, queue_id, max_credit);

    printk(" ti_ppm setting queue %d with max %d\n",queue_id,max_credit);
    return 0;
}
...
/**************************************************************************
 * FUNCTION NAME : ti_ppm_delete_drop_sessions
 **************************************************************************
 * DESCRIPTION   :
 *  The function is used to delete drop sessions created by docsis and netfilter
 * params -
 * netfilter_null_vpid_handle - the netfilter vpid handle
 * docsis_null_vpid_handle - the docsis vpid handle
 * src_ip - the source ip of the ingress session
 * dst_ip - the destination ip of the ingress session
 * src_port - the source port of the ingress session
 * dst_port - the destination port of the ingress session
 * NOTES         :
 *  This function is called from within the Critical Section.
 **************************************************************************/
void ti_ppm_delete_drop_sessions(int *netfilter_null_vpid_handle,int *docsis_null_vpid_handle,unsigned int src_ip,unsigned int dst_ip,
                            unsigned short src_port, unsigned short dst_port)
{
    /* Validate the VPID range before proceeding. */
    ...
    /* Get the pointer to the VPID. */
    if (!((*netfilter_null_vpid_handle < 0) || (*netfilter_null_vpid_handle >= TI_PP_MAX_VPID)))
        {
        ti_ppm_find_and_delete_drop_sessions(netfilter_null_vpid_handle,src_ip,dst_ip,src_port,dst_port);
        }
    /* check for docsis drop sessions*/
    if (!((*docsis_null_vpid_handle < 0) || (*docsis_null_vpid_handle >= TI_PP_MAX_VPID)))
        {
        ti_ppm_find_and_delete_drop_sessions(docsis_null_vpid_handle,src_ip,dst_ip,src_port,dst_port);
        }
     ppm_mcb.os_table.critical_section_end(&os_context);
}
...
{
    PPM_VPID *ptr_vpid;
    ptr_vpid = ppm_mcb.vpid_database[*null_vpid_handle];
    unsigned int session_src_ip, session_dst_ip;
    unsigned short session_src_port, session_dst_port;
    PPM_SESSION_VPID_INFO *temp;
    int session_handle;
    if (ptr_vpid != NULL)
        {
        temp = (PPM_SESSION_VPID_INFO *)list_get_head((LIST_NODE **)&ptr_vpid->vpid_session_mapping);
        while (temp != NULL) 
        {
            if (temp->session->session.ingress.l3l4_packet.packet_type == TI_PP_IPV4_TYPE) 
            {
            session_src_ip = temp->session->session.ingress.l3l4_packet.u.ipv4_desc.src_ip;
            session_dst_ip = temp->session->session.ingress.l3l4_packet.u.ipv4_desc.dst_ip;
            session_src_port = temp->session->session.ingress.l3l4_packet.u.ipv4_desc.src_port;
            session_dst_port = temp->session->session.ingress.l3l4_packet.u.ipv4_desc.dst_port;
			
                session_handle = temp->session->session_handle->handle;
                temp = (PPM_SESSION_VPID_INFO *)list_get_next((LIST_NODE *)temp);

            if (session_src_ip == src_ip && session_dst_ip == dst_ip && session_src_port == src_port && session_dst_port == dst_port)
                {
