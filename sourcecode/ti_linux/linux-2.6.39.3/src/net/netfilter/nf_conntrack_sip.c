...
unsigned int (*nf_not_nat_sip_via_hook)(struct sk_buff *skb,
				unsigned int dataoff,
				const char **dptr,
				unsigned int *datalen) __read_mostly;
EXPORT_SYMBOL_GPL(nf_not_nat_sip_via_hook);
				...
				const char **dptr, unsigned int *datalen)
{
	...
	for (i = 0; i < ARRAY_SIZE(sip_handlers); i++) {
		...
		/**
		 * We have to rewrite udp dest port in any case
		 * when forced_dport is detected
		 */
		if (!(ct->status & IPS_NAT_MASK) &&
				help->help.ct_sip_info.forced_dport) {
			typeof(nf_not_nat_sip_via_hook) nf_not_nat_via;
			nf_not_nat_via = rcu_dereference(nf_not_nat_sip_via_hook);
			if (nf_not_nat_via && !nf_not_nat_via(skb, dataoff, dptr, datalen))
				return NF_DROP;
		}
