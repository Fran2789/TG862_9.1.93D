...
static int _httpd_port = 8081;
static int _httpd_max_connections = 20;
static char _httpd_ip[24]; // UNIHAN ADD
static int _httpd_timeout = 10;
#define HW_MODEL_LEN 12
static char _httpd_hwModel[HW_MODEL_LEN]; // ARRIS ADDED
...
static int _hnap_terminate_signal = SIGUSR1;
...
_httpd_parse_arguments(int argc, char **argv)
{
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    // UNIHAN ADD START
    else if (!strcmp(argv[i - 1], NHTTPD_ARG_IP))
    {
      snprintf(_httpd_ip, sizeof(_httpd_ip), argv[i]);
    }
    // UNIHAN ADD END
    ...
    // ARRIS ADD START
    else if (!strcmp(argv[i - 1], NHTTPD_ARG_HWMODEL))
    {
        snprintf(_httpd_hwModel, HW_MODEL_LEN, argv[i]);
    }
    // ARRIS ADD END
  }
...
httpd_init(int argc, char *argv[])
{
  ...
  return hsocket_bind(&_httpd_socket, _httpd_port, _httpd_ip); // UNIHAN MODIFY
}
...
// ARRIS ADD START
char*
httpd_get_hwModel(void)
{
    return _httpd_hwModel;
}
// ADDRIS ADD END
...
httpd_send_internal_error(httpd_conn_t * conn, const char *errmsg)
{
  ...
  httpd_set_header(conn, HEADER_CONTENT_TYPE, "text/xml"); //ARRIS ADD
  ...
  while (!done)
  {
    ...
    else
    {
      ...
      if ((service = httpd_find_service(req->path)))
      {
        ...
        /* record the result of authorization and handle it after receiving POST data if any */
        rconn->authret = _httpd_authenticate_request(req, service->auth);
...
/* UNIHAN ADD START */
void hnap_term(int sig)
{
	if ( sig == _hnap_terminate_signal )
	{
  		_httpd_run = 0;
	}
}
...
_httpd_register_signal_handler(void)
{
  ...
  signal(_hnap_terminate_signal, hnap_term); /* UNIHAN ADD */
#endif
