...
/* SERCOMM ADD START*/
static unsigned int nf_ct_tcp_timeout_trigger_established __read_mostly =   5 MINS;
/* SERCOMM ADD end*/
		      ...
		      unsigned int hooknum)
{
...
#else
                    if (PP_RC_SUCCESS != avalanche_pp_session_delete(ct->tuplehash[IP_CT_DIR_ORIGINAL].ti_pp_session_handle, NULL))
#endif
...
#else
                    if (PP_RC_SUCCESS != avalanche_pp_session_delete(ct->tuplehash[IP_CT_DIR_REPLY].ti_pp_session_handle, NULL))
#endif
	...
	/* SERCOMM ADD START*/
	if (ct->sectriggermark == 1 && new_state == TCP_CONNTRACK_ESTABLISHED )
		timeout = nf_ct_tcp_timeout_trigger_established;
	else
	/* SERCOMM ADD END*/    
