...
#define IPT_GWMETA_VALID_FLAGS_USED    (1) 
...
{
    struct ipt_gw_skb_rule_info *info = (struct ipt_gw_skb_rule_info *) (*target)->data;
	...
	case '1':  
		...
		info->type = IPT_GW_META;
		*flags |= IPT_GWMETA_VALID_FLAGS_USED;
		break;
	case '2':
		info->type = IPT_GW_DIS_PP;
		*flags |= IPT_GWMETA_VALID_FLAGS_USED;
		break;
	case '3':
		info->type = IPT_GW_EN_PP;
		*flags |= IPT_GWMETA_VALID_FLAGS_USED;
		break;
	default:
		return 0;
      ...
      int numeric)
{
	...
	if (info)
	{
		...
		else if (info->type == IPT_GW_DIS_PP)
		{
			printf("--dis-pp");
		}	
		else if (info->type == IPT_GW_EN_PP)
		{
			printf("--en-pp");
		}		
	}
...
{
	if (!(flags & IPT_GWMETA_VALID_FLAGS_USED))
...
static struct xtables_target gwmeta_tg_reg = {
        ...
        .family         = NFPROTO_IPV4,
        .size           = XT_ALIGN(sizeof(struct ipt_gw_skb_rule_info)),
        .userspacesize  = XT_ALIGN(sizeof(struct ipt_gw_skb_rule_info)),
        .help           = GWMETA_help,
        .parse          = GWMETA_parse,
...
*   man dlopen.
*/
	xtables_register_target(&gwmeta_tg_reg);
}
