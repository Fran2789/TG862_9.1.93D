...
#ifdef INCLUDE_SERCOMM_GW //Sercomm PP enhancement
void (*sc_ppd_session_handler_hook)(TI_PP_SESSION *ptr_session)=NULL;
void (*sc_ppd_clear_session_stats_hook)(Uint8 session_handle)=NULL;
#endif
    ...
    switch(pkt_desc->packet_type)
    {
        case TI_PP_ETH_TYPE:
        {
            ...
            *ram_ptr++ = WORD_B3_0(l2_desc->srcmac[0], l2_desc->srcmac[1], l2_desc->dstmac[0], l2_desc->dstmac[1]);
            ...
            else if ((ingress_vpid->type == TI_PP_VLAN) || (ingress_vpid->type == TI_PP_VLAN_PPPoE))
            {
                enable_flags |= TI_PP_L2_LUT_VLAN_ID_EN;
...
#ifdef INCLUDE_SERCOMM_GW //Sercomm PP enhancement
    if (sc_ppd_session_handler_hook)
        session_flags |= TI_PP_SES_FLAG_PASS_AFTER_TMOUT;
#endif
...
#ifdef INCLUDE_SERCOMM_GW //Sercomm PP enhancement
    if(sc_ppd_session_handler_hook)
	    sc_ppd_session_handler_hook(ses_cfg);
#endif
...
#ifdef INCLUDE_SERCOMM_GW //Sercomm PP enhancement
    if(sc_ppd_clear_session_stats_hook)
	    sc_ppd_clear_session_stats_hook(session_handle);
#endif
...
#ifdef INCLUDE_SERCOMM_GW //Sercomm PP enhancement
EXPORT_SYMBOL(sc_ppd_session_handler_hook);
EXPORT_SYMBOL(sc_ppd_clear_session_stats_hook);
#endif
