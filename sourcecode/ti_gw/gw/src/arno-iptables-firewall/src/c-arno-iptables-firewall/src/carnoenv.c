...
/* ARRIS ADD BEGIN */
#include <sys/ioctl.h>      /* Use getif instead of ifconfig... */
#include <net/if.h>
...
/* ARRIS ADD BEGIN : Cached interface information */
extern int    num_ext;
extern int    num_int;
extern int    num_dmz;
extern int    num_trusted;
extern int*   ext_if_indexes;
extern char** ext_if_strings;
extern int*   int_if_indexes;
extern char** int_if_strings;
extern int*   dmz_if_indexes;
extern char** dmz_if_strings;
extern int*   trusted_if_indexes;
extern char** trusted_if_strings;
extern int    lo_ifidx;
/* ARRIS ADD END */
...
#if 0   /* ARRIS DELETE BEGIN : Use iptables-batch instead of iptable-restore */
...
int get_numeric_ip_version(char *arg1)
{
    ...
    /* ARRIS CHAGNE START */
    /* for possible memory leak and iprange support */
    maybe4 = str = strdup(arg1);
...
#if 0   /* ARRIS MOD BEGIN - replace check_interface with get_interface_index */
/**************************************************************************/
...
#endif /* ARRIS MOD END - replace check_interface with get_interface_index */


/* ARRIS ADD BEGIN - Replace iptables-save/restore with iptables-batch */
FILE* ipv4_rules_file = NULL;
FILE* ipv6_rules_file = NULL;

int ip4tables_batchinit( char* filename )
{
    /*SERCOMM ADD*/
    /*Only setting IPv4 rules, when g_Setting_RuleType is ARNO_SETTING_TYPE_ALL or ARNO_SETTING_TYPE_IPV4*/
    if( (g_Setting_RuleType != ARNO_SETTING_TYPE_ALL) && (g_Setting_RuleType != ARNO_SETTING_TYPE_IPV4)) {
        return 0;
    }
    /*END SERCOMM ADD*/
    ...
    if ( ipv4_rules_file )
    {
        fclose( ipv4_rules_file );
    }
    
    ipv4_rules_file = fopen( filename, "w" );

    return 0;
}


int ip4tables_batchapply( char* filename )
{
    /*SERCOMM ADD*/
    /*Only setting IPv4 rules, when g_Setting_RuleType is ARNO_SETTING_TYPE_ALL or ARNO_SETTING_TYPE_IPV4*/
    if( (g_Setting_RuleType != ARNO_SETTING_TYPE_ALL) && (g_Setting_RuleType != ARNO_SETTING_TYPE_IPV4)) {
    ...
    fclose( ipv4_rules_file );
    ipv4_rules_file = NULL;

    /* calculate needed size, not including trailing NULL */
    szneeded = snprintf( cmdstring, sizeof(cmdstring), "iptables-batch %s", filename );;
    if (szneeded++ >= sizeof(cmdstring))
    {
        printf("iptables_batchapply - more buffer requested than available\n");
        char *buf = (char*)calloc(1, szneeded);
        if (!buf)
        {
            printf("iptables_batchapply - failed to allocate memory!\n");
            return -1;
        }
        snprintf(buf, szneeded, "iptables-batch %s", filename);
        system(buf);
        free(buf);
        return 0;
    }
...
int ip4tables_batch( char* arg1 )
{
    ...
    if ( ipv4_rules_file )
    {
        fprintf( ipv4_rules_file, "iptables %s\n", arg1 );
    }
    
    return 0;
}

int ip6tables_batchinit( char* filename )
{
    /*SERCOMM ADD*/
    /*Only setting IPv6 rules, when g_Setting_RuleType is ARNO_SETTING_TYPE_ALL or ARNO_SETTING_TYPE_IPV6*/
    if( (g_Setting_RuleType != ARNO_SETTING_TYPE_ALL) && (g_Setting_RuleType != ARNO_SETTING_TYPE_IPV6)) {
        return 0;
    }
    /*END SERCOMM ADD*/
    ...
    if ( ipv6_rules_file )
    {
        fclose( ipv6_rules_file );
    }
    
    ipv6_rules_file = fopen( filename, "w" );

    return 0;
}


int ip6tables_batchapply( char* filename )
{
    /*SERCOMM ADD*/
    /*Only setting IPv6 rules, when g_Setting_RuleType is ARNO_SETTING_TYPE_ALL or ARNO_SETTING_TYPE_IPV6*/
    if( (g_Setting_RuleType != ARNO_SETTING_TYPE_ALL) && (g_Setting_RuleType != ARNO_SETTING_TYPE_IPV6)) {
    ...
    fclose( ipv6_rules_file );
    ipv6_rules_file = NULL;
    
    /* calculate needed size, not including trailing NULL */
    szneeded = snprintf( cmdstring, sizeof(cmdstring), "ip6tables-batch %s", filename );;
    if (szneeded++ >= sizeof(cmdstring))
    {
        printf("ip6tables_batchapply - more buffer requested than available\n");
        char *buf = (char*)calloc(1, szneeded);
        if (!buf)
        {
            printf("ip6tables_batchapply - failed to allocate memory!\n");
            return -1;
        }
        snprintf(buf, szneeded, "ip6tables-batch %s", filename);
        system(buf);
        free(buf);
        return 0;
    }
...
int ip6tables_batch( char* arg1 )
{
    ...
    if ( ipv6_rules_file )
    {
        fprintf( ipv6_rules_file, "ip6tables %s\n", arg1 );
    }
    
    return 0;
}

int ip4tables_batchdeinit()
{
    if ( ipv4_rules_file )
    {
        fclose( ipv4_rules_file );
...
#if 0 /* ARRIS DELETE BEGIN - USE simpler iptable-batch instead of iptables-save/restore */
/**************************************************************************/
...
//            shift 2
        ...
        //if [ -n "$first_param" ]; then
        /* ARRIS MOD START */
        /* Remove duplicate chain checking */
        #if 0
        if  (  ((first_param!=NULL)&&(strlen(first_param)!=0))  )
        {
...
#if 0 /* ARRIS MOD : just call new ip4tables_batch() */
    if (strcmp(FORCE_IP4TABLESBATCH, "1") == 0)
    {
...
#endif /* ARRIS DELETE END - USE simpler iptable-batch instead of iptables-save/restore */
...
#if 0 /* ARRIS MOD BEGIN : Just call ip6tables_batch */
    if (strcmp(FORCE_IP6TABLESBATCH, "1") == 0)
    {
    ...
    for (arg = strtok_r(usearg, IFS, &r_arg1); arg != NULL; arg = strtok_r(NULL, IFS, &r_arg1))
    {
      ...
      else
      {
        ...
        if ((strcmp(arg, "-s") == 0) || (strcmp(arg, "--source") == 0) 
            || (strcmp(arg, "--src-range") == 0)) /* ARRIS CHAGNE to support iprange */
        {
          //action="s"
            action='s';
          //;;
        ...
        if ((strcmp(arg, "-d") == 0) || (strcmp(arg, "--destination") == 0) 
            || (strcmp(arg, "--dst-range") == 0)) /* ARRIS CHANGE to support iprange */
        {
          //action="d"
            action='d';
          //;;
...
#else
int sysctl(char *key, char* value)
{
    char  proc_path[256];
    FILE* fd;
    int   retval = -1;
    ...
    fd = fopen( proc_path, "w" );

    if ( fd )
    {
        if ((value[0] != '"') && (strchr(value, ' ') != NULL))
        {
            retval = ( fprintf( fd, "\"%s\"", value ) > 0 ) ? 0 : -1;
        }
        else
        {
            retval = ( fprintf( fd, "%s", value ) > 0 ) ? 0 : -1;
...
//{
{
    ...
    char *fld = NULL;
    char *tr1_FREE = NULL;
    char *orig_VARIABLES = NULL; /*SERCOMM ADD*/
    ...
    orig_VARIABLES = strdup(VARIABLES); /*SERCOMM ADD*/
    ...
    tr1_FREE = tr(orig_VARIABLES, " ", "|"); /*SERCOMM MODIFY*/
...
ext:
    ...
    //if (tr1_FREE != NULL) free(tr1_FREE); /*SERCOMM REMOVE*/
    FREE(sysctl_commandline);
    FREE(OPTIONS);
    FREE(result);
    FREE(VARIABLES);
    ...
    FREE(fld);
    FREE(tr1_FREE);
    FREE(orig_VARIABLES); /*SERCOMM ADD*/
    return retval;
//}
}
#endif
...
int sysctl_set_all(char *prefix, char *variable, char *value)
{
  ...
  // ARRIS MERGE REMOVE
  //IFS1=EOL;
  IFS_1=EOL; /*SERCOMM MODIFY: using IFS_1 not IFS, because IFS will be used in another for loop*/

  //for line in $(echo $SYSCTL_ALL |cut -d' ' -f1 |grep "^${prefix}\..*\.${variable}$"); do
  sysctl = strdup(SYSCTL_ALL);
  pre = stringcat(prefix, ".", NULL);
  prelen = strlen(pre); /*SERCOMM MODIFY*/
  var = stringcat(".", variable, NULL);
  varlen = strlen(var); /*SERCOMM MODIFY*/
  for (line = strtok_r(sysctl, IFS_1, &r_line); line != NULL; line = strtok_r(NULL, IFS_1, &r_line)) /*SERCOMM MODIFY: using IFS_1*/
  {
      FREE(fld);
      fld = getfield(line, " ", 1);
      if ((fld != NULL) && (strncmp(fld, pre, prelen) == 0) && (strncmp(fld + strlen(fld) - varlen, var, varlen) == 0))
      {
          ...
          for (interface = strtok_r(interfaces, IFS2, &r_interface); interface != NULL; interface = strtok_r(NULL, IFS2, &r_interface))
          {
              ...
              if ((fld != NULL) && (cmp != NULL) && (strcmp(fld, cmp) == 0)) /*SERCOMM MODIFY*/
              {
                  //      sysctl -w "$line=$value"
                  FREE(cmd);
                  cmd = stringcat(SYSCTL, " -w ", fld, "=", value, NULL); /*SERCOMM MODIFY*/
                  FREE(res);
...
int sysctl_set_all(char *prefix, char *variable, char *value)
{
    ...
    for ( i=0; i<num_ext; i++)
    {
        if ( ext_if_indexes[i] >= 0 )
        {
            snprintf( sysctl_path, sizeof(sysctl_path), "%s/%s/%s", prefix, ext_if_strings[i], variable );
            retval |= sysctl( sysctl_path, value ); 
        }
    }
    
    for ( i=0; i<num_int; i++)
    {
        if ( int_if_indexes[i] >= 0 )
        {
            snprintf( sysctl_path, sizeof(sysctl_path), "%s/%s/%s", prefix, int_if_strings[i], variable );
            retval |= sysctl( sysctl_path, value ); 
        }
    }
    ...
    for ( i=0; i<num_dmz; i++)
    {
        if ( dmz_if_indexes[i] >= 0 )
        {
            snprintf( sysctl_path, sizeof(sysctl_path), "%s/%s/%s", prefix, dmz_if_strings[i], variable );
            retval |= sysctl( sysctl_path, value ); 
        }
    }
...
int sysctl_set_ext(char *prefix, char *variable, char *value)
{
    char  sysctl_path[256];
    int   retval = 0;
    int   i;
    ...
    for (item = strtok_r(usearg, IFS, &r_arg); item != NULL; item = strtok_r(NULL, IFS, &r_arg))
    {
        ...
        if ((start > 0) && (stop > 0))
        {
            for (i = start; i <= stop; i++)
            {
                ...
                snprintf(ip4, sizeof(ip4), "%s%d%s", host_base, i, FIRST == 1 ? "" : ",");
                FIRST = 0;
                p = result;
...
unsigned int get_ipv4_netaddr(const char *ifname)
{
    int fd;
    struct ifreq ifr;
...
char* get_network_ipv4_address(char *ifname)
{
    ...
    char *ip;
    int ip_len;
    ip_len = 18*sizeof(char);
    ip = malloc(ip_len);

    saddr.sin_addr.s_addr = get_ipv4_netaddr(ifname);
    strncpy(ip, inet_ntoa(saddr.sin_addr), ip_len);

    return ip;
}

unsigned int get_ipv4_netmask(const char* if_name)
{
    int retval;
    int fd = socket(AF_INET, SOCK_DGRAM, 0);
    if (fd < 0) { perror("get_ip4_netmask - socket"); return 0; }
    ...
    retval = ioctl(fd, SIOCGIFNETMASK, &req);
    
    close(fd);

    return (retval == 0 ? saddr->sin_addr.s_addr : 0);
}
...
char* get_network_ipv4_address_mask(char *ifname)
{
#if 0
    char *cmd;
    int  ifname_len = ((ifname != NULL) ? strlen(ifname) : 0);
    int  cmd_len = 128 + ifname_len;
    char *ret;
    cmd = malloc(cmd_len);
    snprintf(cmd, cmd_len, "ip -o addr show dev \"%s\" 2>/dev/null |awk '$3 == \"inet\" { printf \"%%s\",$4; exit; }'", ifname);
    ret = sysout(cmd, NULL);
    free(cmd);
#else
    unsigned int addr, mask, prefix, len;
    struct sockaddr_in saddr;
    char *ret;
    const int ipv4_buf_size = 24;

    addr = get_ipv4_netaddr(ifname);
    mask = get_ipv4_netmask(ifname);

    /* netmask to prefix */
    mask = ntohl(mask);
    prefix = 0;
    while( !((mask >> prefix) & 1)  && (prefix < 32))
    {
        ++prefix;
    }
    prefix = 32 - prefix;
    ...
    ret = (char*)calloc(ipv4_buf_size, 1);
    strncpy(ret, inet_ntoa(saddr.sin_addr), ipv4_buf_size - 1);
    len = (unsigned int)strlen(ret);
    snprintf(ret + len, ipv4_buf_size - len, "/%d", prefix);
#endif
    return ret;
}


static void ipv6_netmask_2_prefix(struct in6_addr *netmask, int *prefix)
{
    int i = 0, pos = 15, ci = 0;
    unsigned char *pc = (unsigned char*)netmask;
    ...
    /* How many trailing 0s */
    while (pos >= 0)
    {
        ci = 0;
        while( !(((*(pc + pos)) >> ci) & 1)  && (ci < 8))
        {
            ++ci;
        }
        i += ci;
...
static int get_ipv6_netaddr_prefix(char *ifname, struct in6_addr *addr6, int *prefix,
        struct in6_addr *lladdr6, int *llprefix)
{
    struct ifaddrs *ifaddr, *ifa;
    struct sockaddr_in6 addr;
    struct in6_addr mask, lmask;
    int retval = -1;

    memset(&mask, 0, sizeof(struct in6_addr));
    memset(&lmask, 0, sizeof(struct in6_addr));

    if (addr6) memset(addr6, 0, sizeof(struct in6_addr));
    if (prefix) *prefix = 0;
    if (lladdr6) memset(lladdr6, 0, sizeof(struct in6_addr));
    if (llprefix) *llprefix = 0;
    
    if (getifaddrs(&ifaddr) == -1)
    {
        perror("get_ip6_addr - getifaddrs\n");
        return -1;
    }
    ...
    for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next)
    {
        ...
        if (IN6_IS_ADDR_LINKLOCAL(&addr.sin6_addr) && (lladdr6 || llprefix))
        {
            if (lladdr6)
            {
                memcpy(lladdr6, &addr.sin6_addr, sizeof(struct in6_addr));
            }

            if (llprefix)
            {
                memcpy(&lmask, &((struct sockaddr_in6 *)ifa->ifa_netmask)->sin6_addr, sizeof(struct in6_addr));
            }

            retval = 0;
        }
        else if (!IN6_IS_ADDR_LINKLOCAL(&addr.sin6_addr) && (addr6 || prefix))
        {
            if (addr6)
            {
                memcpy(addr6, &addr.sin6_addr, sizeof(struct in6_addr));
            }

            if (prefix)
            {
                memcpy(&mask, &((struct sockaddr_in6 *)ifa->ifa_netmask)->sin6_addr, sizeof(struct in6_addr));
            }
    ...
    if (prefix)
    {
        ipv6_netmask_2_prefix(&mask, prefix);
    }
    if (llprefix)
    {
        ipv6_netmask_2_prefix(&lmask, llprefix);
    }
    return retval;
}
...
char* get_network_ipv6_address(char *ifname)
{
    ...
    char *cmd;
    int  ifname_len = ((ifname != NULL) ? strlen(ifname) : 0);
    int  cmd_len = 256 + ifname_len;
    char *ret;
    cmd = malloc(cmd_len);
    snprintf(cmd, cmd_len, "ip -o addr show dev \"%s\" 2>/dev/null | awk '$3 == \"inet6\" { printf \"%%s\",$4; exit; }' | awk -F \"/\" '{ printf \"%%s\",$1; exit; }'", ifname);
    ret = sysout(cmd, NULL);
    free(cmd);
#else
    struct in6_addr addr, lladdr, *paddr;
    struct in6_addr nulladdr;
    char *ret;
    int ip6len = 48;
    int prefix, llprefix;
    memset(&nulladdr, 0, sizeof(struct in6_addr));
    get_ipv6_netaddr_prefix(ifname, &addr, &prefix, &lladdr, &llprefix);

    /* "ip -o addr show dev \"%s\" 2>/dev/null | awk '$3 == \"inet6\" { printf \"%%s\",$4; exit; }' | awk -F \"/\" '{ printf \"%%s\",$1; exit; }'" */
    /* fetch global ipv6 address */
    /* if no global ipv6 address, fetch the link local ipv6 address */
    if (memcmp(&addr, &nulladdr, sizeof(struct in6_addr)))
    {
        paddr = &addr;
    }
    else if (memcmp(&lladdr, &nulladdr, sizeof(struct in6_addr)))
    {
        paddr = &lladdr;
    }
    else
    ...
    ret = (char*)calloc(ip6len, 1);
    inet_ntop(AF_INET6, paddr, ret, ip6len); 
#endif
    return ret;
}
/*SERCOMM ADD END*/
