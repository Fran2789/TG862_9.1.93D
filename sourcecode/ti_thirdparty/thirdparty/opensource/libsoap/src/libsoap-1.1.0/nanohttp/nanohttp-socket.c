...
----------------------------------------------------*/
herror_t
hsocket_bind(hsocket_t * dsock, int port, char *ip) // UNIHAN MODIFY
{
  hsocket_t sock;
  ...
  {
	log_error2("Cannot setsockopt (%s)", strerror(errno));
  ...
  addr.sin_addr.s_addr = inet_addr(ip); // UNIHAN MODIFY
