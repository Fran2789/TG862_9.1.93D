...
#include <asm-arm/arch-avalanche/puma6/docsis_ip_boot_params.h>
#endif
...
static int __devinit sdhci_pltfm_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_ARCH_GEN3
	/* Select Host controller base address to use Phisical or Virtual AEP*/
    if (aep_is_active() != 0)
	{
		// Use AEP virtual host controller
		// printk("[PowerOn] sdhci_alloc_host() Use AEP virtual host controller. \n");
		iomem = platform_get_resource(pdev, IORESOURCE_MEM, 1);
	}
    else
	{
#endif
		// Use physical host controller
		// printk("[PowerOn] sdhci_alloc_host() Use physical host controller. \n");
    ...
    // printk("[PowerOn] sdhci_pltfm_probe iomem->start = %p\n",iomem->start); 
...
*/
    ...
    SRAM_Get_Boot_Param(NPCPU_ONLY_MODE_ID ,&np_only_mode);
    if (np_only_mode == NPCPU_ONLY_ENABLE)
    {
        printk(KERN_INFO"CPU boot mode is NPCPU_ONLY_ENABLE\n",np_only_mode);
    }
    else
    {
	...
	do
	{
		res = arm_atom_mbx_receive_event_notification(ATOM_EVENT_EMMC_ADVANCE_EXIT,NULL);
		if (res)
			printk(KERN_INFO"[%s] NP-CPU MBOX  -  did not get event 'eMMC advance mode ready' notification from APP-CPU, retrying ... \n", __FUNCTION__);
		loops--;
	}while ((res != 0) && loops);

	if (res != 0)
	{
		printk(KERN_ERR" NP-CPU MBOX error  -  did not get event 'eMMC advance mode ready' notification from APP-CPU \n\n");
		printk(KERN_INFO"  +----------------------------------------------------------------------------+  \n");
