...
/* SERCOMM MODIFY */
        ...
        {
	        table = fib6_get_table(dev_net(dev), GW_ROUTING_TABLE);
        }
        else
        {
...
struct rt6_info *rt6_get_dflt_router_from_table(struct in6_addr *addr, struct net_device *dev, u32 table_index)
{
        struct rt6_info *rt;
        struct fib6_table *table;

        table = fib6_get_table(dev_net(dev), table_index);
...
struct rt6_info *rt6_add_dflt_router_to_table(struct in6_addr *gwaddr,
                                     struct net_device *dev,
                                     unsigned int pref,
                                     u32 table_index
                                     )
{
        ...
        ip6_route_add(&cfg);
	return rt6_get_dflt_router_from_table(gwaddr, dev, table_index);
}
...
restart:
   ...
   //ARRIS ADD - Start - PROD00205112
   if((rt->rt6i_dev->name[0]=='w' && rt->rt6i_dev->name[1]=='a' && rt->rt6i_dev->name[2]=='n' && rt->rt6i_dev->name[3]=='0') ||
           (rt->rt6i_dev->name[0]=='m' && rt->rt6i_dev->name[1]=='t' && rt->rt6i_dev->name[2]=='a' && rt->rt6i_dev->name[3]=='0'))
   {
       continue;
   }
   //ARRIS ADD - End
