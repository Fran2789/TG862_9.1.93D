...
/* ARRIS ADD BEGIN : Use spinlocks to protect gpio regs, not semaphore */
...
static int readGpioProc(char*, char**, off_t, int, int*, void*);
...
INT32 PAL_sysGpioCtrlSetDir(UINT32 gpio_pin, PAL_SYS_GPIO_PIN_DIRECTION_T pin_direction)
{
    ...
    /* Critical_Start */
    // down(&gpioSem);          // ARRIS REMOVE

/* ARRIS CHANGE - I've added the ARM HW MUTEX lock/unlock patches in hw_mutex.c, but...
                - only Comcast RDK 1.3 builds as of Sept 18, 2013 have the GPIO HW MUTEX patch in the ATOM kernel.
                - The Atom kernel patch MUST be present in order to call the lock/unlock HW MUTEX functions on ARM.
                - This was added to Atom in Rel 4.2.3.58.  As soon as we merge that into Atom, we can modify these
                - compile flags to enable the HW mutex calls for all FW load types.
END ARRIS CHANGE */
#if ( defined(CONFIG_VENDOR_ARRIS_RDK) && defined(INCLUDE_CT) )
    /* Take the HW mutex */
    ...
    spin_lock_irqsave(&gpio_spinlock, flags);       // ARRIS ADD
...
// ARRIS ADD
/*! \fn INT32 PAL_sysGpioCtrl(PAL_SYS_GPIO_PIN_DIRECTION_T pin_direction)
    \brief This API initializes the mode of a specific GPIO pin
    \param gpio_pin GPIO pin number 
    \param pin_direction Direction of the gpio pin in GPIO_PIN mode
    \return Returns (0) on success and (-1) on failure
*/
INT32 PAL_sysGpioCtrl(UINT32 gpio_pin,PAL_SYS_GPIO_PIN_DIRECTION_T pin_direction)
{
    UINT32 cookie;
    UINT32 gpio_out_en_reg;
    unsigned long flags;
...
static int __init gpio_drv_init(void)
{
	...
	/* Creating proc file */
	procFile = create_proc_entry(GPIO_PROC_NAME, 0644, NULL);
    ...
    procFile->read_proc = readGpioProc;
...
static int readGpioProc(char *page, char **start, off_t off, int count,int *eof, void *data_unused)
{
	int len     = 0;
    int l       = 0;
    off_t begin = 0;
