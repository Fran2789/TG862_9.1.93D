...
class TSrvOptIA_NA : public TOptIA_NA
{
 ...
 private:
    ...
    bool checkReservedSubnet(SPtr<TIPv6Addr> addr ,std::string iface_name );
    int getPrefixSubnetLength( std::string iface_name );
    int getIntferNumber( std::string name );
    bool isDuplicateReservedSubnet( int length  , std::string addr );
    std::string ConvertIPv6ToBinary( std::string ipv6_addr );
    // UNIHAN ADD END ,PROD00200920

    SPtr<TIPv6Addr>   ClntAddr;
    SPtr<TDUID>       ClntDuid;
    int               Iface;
    int               MsgType;	// UNIHAN ADD , PROD00225555
