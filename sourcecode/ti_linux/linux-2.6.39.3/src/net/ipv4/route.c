...
//ARRIS ADD START
ARRISMOD_DEFINE( int, arris_rip_subnet_check_hook, unsigned int );
//ARRIS ADD END
...
struct rtable *ip_route_output_flow(struct net *net, struct flowi4 *flp4,
				    struct sock *sk)
{
    // ARRIS ADD - for sockets bound to all interfaces, let's try to reverse-lookup
    // which interface to use based upon the local IP address. Otherwise the kernel
    // will decide for us, and may end up choosing to use the MTA IF for WAN packets,
    // or vice versa. Both interfaces have default routes to the internet.
    if( flp4->flowi4_oif == 0 )
    {
        if (flp4->saddr)
        {
            struct net_device *dev_out = NULL;

            dev_out = ip_dev_find(net, flp4->saddr);
            if( dev_out )
            {
#ifdef INCLUDE_SERCOMM_GW
                unsigned int from_gw_ip = 0;

                /* add extra checks to fix RIP GW ping issue */
                if (strcmp("br0",dev_out->name) == 0)
                {
                    /* don't re-direct packets coming from the RIP GW IP (br0:0), let the stack assign the interface */
                    struct ifreq ifr;
                    struct sockaddr_in *sin = (struct sockaddr_in *)&ifr.ifr_addr;

                    /* get the ip address of the RIP GW interface */
                    strcpy(ifr.ifr_name, "br0:0");
                    if ( devinet_ioctl_non_blocking(SIOCGIFADDR,&ifr) < 0 )
                    {
                        /* Unable to retrieve the RIP GW address, set it to 0 */
                        sin->sin_addr.s_addr = 0;
                    }

                    if ( sin->sin_addr.s_addr == flp4->saddr )
                    {
                        /* this packet is from the GW RIP IP, don't redirect to br0 */
						from_gw_ip  = 1;
                    }
		}

		/* assign interface if packet is not from the GW RIP IP */
		if(from_gw_ip == 0) 
		{
		    flp4->flowi4_oif = dev_out->ifindex;
		}
		...
		/* do not re-direct output for lan0 packets.  it blocks access to the 192.168.100.1 network from
		   the routed interface.
		   Long term solution will be to use policy routing for the mta so that we can completely remove
		   this code */
        // UNIHAN MOD START
        int from_gw_ip = 0;
		...
		if (strcmp("l2sd0.2",dev_out->name) == 0)
#else
        if (strcmp("eth0.2",dev_out->name) == 0)
#endif
		{
			/* don't re-direct packets coming from the RIP GW IP (l2sd0.2:0), let the stack assign the interface */
			struct ifreq ifr;
			struct sockaddr_in *sin = (struct sockaddr_in *)&ifr.ifr_addr;
			...
			/* get the ip address of the RIP GW interface */
#ifdef CONFIG_MACH_PUMA6
			strcpy(ifr.ifr_name, "l2sd0.2:0");
#else
            strcpy(ifr.ifr_name, "eth0.2:0");
#endif
			if ( devinet_ioctl_non_blocking(net,SIOCGIFADDR,&ifr) < 0 )
			{
				/* Unable to retrieve the RIP GW address, set it to 0 */
				sin->sin_addr.s_addr = 0;
			}
			...
			if ( sin->sin_addr.s_addr == flp4->saddr )
			{
				/* this packet is from the GW RIP IP, don't redirect to l2sd0.2 */
				from_gw_ip  = 1;
			}
		}
                /* add extra checks to fix RIP CPE access GW WAN interface issue */
                if(strcmp("erouter0",dev_out->name) ==0 )
                {
                      unsigned int        rip_addr=0;
                      unsigned int        rip_mask=0;
                      struct ifreq ifr;
                      struct sockaddr_in *sin = (struct sockaddr_in *)&ifr.ifr_addr;
                     /* get the ip address of the RIP GW interface */
#ifdef CONFIG_MACH_PUMA6
                      ...
                      strcpy(ifr.ifr_name, "eth0.2:0");
                       ...
                       {
                            rip_addr = sin->sin_addr.s_addr;
                            /* get the netmask  of the RIP GW interface */
                            if( devinet_ioctl_non_blocking(net,SIOCGIFNETMASK,&ifr) < 0 )
                            {
                                rip_mask=0;
                            }
                            else
                            {
                                rip_mask= sin->sin_addr.s_addr;
                                if( (rip_mask !=0) && (rip_addr!=0) && ((rip_addr & rip_mask) == (flp4->daddr & rip_mask)) )
                                {
                                    //NA load is using erouter0 as RIP interface, make sure we don't assign the interface for replies from erouter0 to rip cpe
                                    from_gw_ip  = 1;
                                }
                            }
		...
		// make sure we don't assign the interface for replies from wan0 to rip cpe
		if(strcmp("wan0",dev_out->name) ==0 ){			
			if(arris_rip_subnet_check_hook && arris_rip_subnet_check_hook( flp4->daddr )){
				from_gw_ip  = 1;
			}
		}
        ...
        flp4->flowi4_oif = dev_out->ifindex;
        dev_put( dev_out );
        #endif // end of INCLUDE_ARRIS_GW
#endif// end of INCLUDE_SERCOMM_GW
        // UNIHAN MOD END
            }
        }
    }
