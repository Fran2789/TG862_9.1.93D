...
#define N25Q064_SECT_SIZE       (64*1024) /* ARRIS ADD */
...
#define N25Q064_ID				(0xBA17) /* ARRIS */
...
#define N_N25Q064_SECTORS		( 128) /* ARRIS */
...
#define N25Q064_PROT_RATIO	 			(0xFFFFFFFF) /* ARRIS */
...
static struct mtd_erase_region_info n25q64_regions =
	{0, N25Q064_SECT_SIZE, N_N25Q064_SECTORS}; // ARRIS ADD
...
static struct sfl_flash_info sfl_data [] = {
	...
	{ "n25q128", m25flxxxx_probe, N25Q128_ID, N25Q128_SECT_SIZE,
            ...
            "NUMONYX2" }, /* ARRIS differentiate Numonyx support added after initial parts above */

	{ "n25q64", m25flxxxx_probe, N25Q064_ID, N25Q064_SECT_SIZE,
			N_N25Q064_SECTORS, N25QXXXXX_PAGE_SIZE, 0x3C, 1,
			N25Q064_PROT_RATIO, &n25q64_regions, &n25qxxxx_cmd_table, N25QXXXXX_SECT_PER_BLOCK,
            "NUMONYX2" }, /* ARRIS ADD, and differentiate Numonyx support added after initial parts above */
    ...
    { "mx25l6405d", mx25lxxxx_probe, MX25L6405_ID, MX25LXXXX_SECT_SIZE,
    ...
    { "mx25l12805d", mx25lxxxx_probe, MX25L12805_ID, MX25LXXXX_SECT_SIZE,
...
/* ARRIS ADD BEGIN*/
static char *flashpartinfo = "", *flashmfginfo = "";
/* ARRIS ADD END*/
						  ...
						  size_t *retlen, u_char *buf)
{
	...
	unsigned char old_mode,temp, read_cmd;	//ARRIS ADD
	int ret = 0;
#ifdef MONOLITHIC_32MB_FLASH
    int fl_tst_res = 0; 
    size_t fl_retlen = 0;
							   ...
							   size_t *retlen, const u_char *buf)
{
    ...
    if( ( to + len ) > flash->mtd.size){
		PRINTK("length (to=%08x, len=%zd) exceeds partition size (%zd)\n", (unsigned int)to, len, flash->mtd.size); // ARRIS ADD more data
		return -EINVAL;
	}
...
/* ARRIS ADD BEGIN*/
static int 
flash_ver_info(char *buf, char **start, off_t offset, int count, int *eof, void *data)    
{                                                                                              
	int len=0;                                                                                 
                                                                                               
	len += sprintf(buf +len,"%s %s\n",flashmfginfo, flashpartinfo);         
	...
	for(i = 0; i < ARRAY_SIZE(sfl_data); i++)
	{
		if(sfl_data[i].probe)
		{
			if(sfl_data[i].probe(spi,i))
			{
                ...
                /* ARRIS ADD BEGIN*/
                flashpartinfo = sfl_data[i].name;
                flashmfginfo  = sfl_data[i].mfg_name;
                /* ARRIS ADD END*/
				flash_found 	= 1;
				index 			= i;
				break;
	...
	if( mtd_has_partitions() )
	 {
		...
		if (nr_parts > 0) {
			for (i = 0; i < nr_parts; i++){
				printk( KERN_ERR "partitions[%d] = ""{.name = %s, .offset = 0x%.8llx," /* ARRIS change to ERR so it prints on gateways */
                      ".size = 0x%.8llx (%lluK) }\n",i, parts[i].name,
...
static int sfl_init( void )
{
    ...
    /*ARRIS ADD BEGIN*/
    create_proc_read_entry("flashinfo",0, NULL, flash_ver_info, NULL); 
    /*ARRIS ADD END*/
...
static void sfl_exit( void )
{
    ...
    /*ARRIS ADD BEGIN*/
    remove_proc_entry("flashinfo", NULL); 
    /*ARRIS ADD END*/
}
