 ...
 * Changed/modified portions - Copyright © 2012 - 2014 , Intel Corporation.   
 */ 
...
#include <asm-arm/arch-avalanche/puma6/aep.h>
#endif
#include "sdhci.h"
...
static void sdhci_reset(struct sdhci_host *host, u8 mask)
{
    ...
    // printk("[PowerOn] sdhci_reset: sdhci_writeb(host, %d, SDHCI_SOFTWARE_RESET)\n",mask);
	sdhci_writeb(host, mask, SDHCI_SOFTWARE_RESET);

	if (mask & SDHCI_RESET_ALL)
    {
        // printk("[PowerOn] sdhci_reset(ALL): set host->clock to %d\n",0);
		host->clock = 0;
    }
...
static void sdhci_init(struct sdhci_host *host, int soft)
{
	...
	if (soft)
    {
        // printk("[PowerOn] sdhci_init() call sdhci_reset(host, SDHCI_RESET_CMD|SDHCI_RESET_DATA);\n");
		sdhci_reset(host, SDHCI_RESET_CMD|SDHCI_RESET_DATA);
    }
    else
    {
        // printk("[PowerOn] sdhci_init() call sdhci_reset(host, SDHCI_RESET_ALL);\n");
	...
	if (soft) {
		...
		host->clock = 0;
        // printk("[PowerOn] sdhci_init() call sdhci_set_ios\n");
...
static u8 sdhci_calc_timeout(struct sdhci_host *host, struct mmc_data *data)
{
    ...
    {
        // printk("[PowerOn] sdhci_calc_timeout() SDHCI_QUIRK_BROKEN_TIMEOUT_VAL is set, return 14\n");
		return 0xE;
    }
...
static void sdhci_prepare_data(struct sdhci_host *host, struct mmc_data *data)
{
    ...
    // printk("[PowerOn] sdhci_prepare_data: set Data timeout counter to %d (max=14)\n",count);
...
static void sdhci_finish_data(struct sdhci_host *host)
{
	...
	if (data->stop) {
        ...
        // printk("[PowerOn] sdhci_finish_data: sdhci_send_command(host, data->stop)\n");
		sdhci_send_command(host, data->stop);
	} else
		tasklet_schedule(&host->finish_tasklet);
    // printk("[PowerOn] sdhci_finish_data: done\n");
}
...
static void sdhci_send_command(struct sdhci_host *host, struct mmc_command *cmd)
{
    ...
    // printk("[PowerOn] sdhci_send_command: command flags=0x%.8X. (cmd=%d, crc=%d, data=%d, reponse=%d)\n",flags, ((flags&SDHCI_CMD_INDEX)!=0),((flags&SDHCI_CMD_CRC)!=0),((flags&SDHCI_CMD_DATA)!=0),flags&(SDHCI_CMD_RESP_NONE|SDHCI_CMD_RESP_LONG|SDHCI_CMD_RESP_SHORT_BUSY|SDHCI_CMD_RESP_SHORT));


    if ((cmd->flags & MMC_RSP_BUSY) && (!(cmd->data)))
    {
        // printk("[PowerOn] sdhci_send_command: set Data timeout counter to %d (max=14)\n",0xE);
	    sdhci_writeb(host, 0xE, SDHCI_TIMEOUT_CONTROL);
    }
    // printk("[PowerOn] sdhci_send_command: SDHCI_COMMAND = %d\n",cmd->opcode);
	sdhci_writew(host, SDHCI_MAKE_CMD(cmd->opcode, flags), SDHCI_COMMAND);
    // printk("[PowerOn] sdhci_send_command: done.\n");
}
...
static void sdhci_finish_command(struct sdhci_host *host)
{
    ...
    {
        // printk("[PowerOn] sdhci_finish_command: calling sdhci_finish_data.\n");
...
static void sdhci_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	{
		// Do not poll Clock Control in AEP
		printk(KERN_WARNING "%s: Do not poll Clock Control in AEP mode - it will never get 'Stable' .\n", mmc_hostname(host->mmc));
	}
...
out:
	host->clock = clock;
    // printk("[PowerOn] sdhci_set_clock: set host->clock to %d\n",host->clock);
}
...
static void sdhci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
    ...
    // printk("[PowerOn] sdhci_request called\n");
    ...
    // printk("[PowerOn] sdhci_request card present state 0%.8X\n", sdhci_readl(host, SDHCI_PRESENT_STATE));
	if (!present || host->flags & SDHCI_DEVICE_DEAD)
    {
        // printk("[PowerOn] sdhci_request tasklet_schedule(&host->finish_tasklet)\n");
		host->mrq->cmd->error = -ENOMEDIUM;
		tasklet_schedule(&host->finish_tasklet);
	} 
    else
    {
        // printk("[PowerOn] sdhci_request -> sdhci_send_command()\n");
...
static void sdhci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
    ...
    // printk("[PowerOn] sdhci_set_ios called\n");
	host = mmc_priv(mmc);
...
static void sdhci_timeout_timer(unsigned long data)
{
	...
	if (host->mrq) {
		...
		if (host->data) {
            ...
            // printk("[PowerOn] sdhci_timeout_timer: calling sdhci_finish_data.\n");
			sdhci_finish_data(host);
		} else {
			if (host->cmd)
...
static void sdhci_data_irq(struct sdhci_host *host, u32 intmask)
{
		...
		if (host->cmd && (host->cmd->flags & MMC_RSP_BUSY)) 
        {
            ...
            else if (intmask & SDHCI_INT_DATA_TIMEOUT)
            {
                printk(KERN_ERR "%s: Got data interrupt 0x%08x SDHCI_INT_DATA_TIMEOUT "
                       " while waiting to Command with BUSY flag to finish.\n",mmc_hostname(host->mmc), (unsigned)intmask);
				return;
			}
		}

		printk(KERN_ERR "%s: Got data interrupt 0x%08x even "
			"though no data operation was in progress. (CMD%d)\n",
			mmc_hostname(host->mmc), (unsigned)intmask, host->mrq->cmd->opcode);
		sdhci_dumpregs(host);
	...
	if (host->data->error)
    {
        // printk("[PowerOn] sdhci_data_irq: calling sdhci_finish_data(host), due to an error\n");
		sdhci_finish_data(host);
    }
    else 
    {
		...
		if (intmask & SDHCI_INT_DATA_END) {
			if (host->cmd) {
                ...
                // printk("[PowerOn] sdhci_data_irq: got SDHCI_INT_DATA_END, calling sdhci_finish_data(host)\n");
				sdhci_finish_data(host);
			}
		}
	}
    // printk("[PowerOn] sdhci_data_irq: done\n");
}
...
static irqreturn_t sdhci_irq(int irq, void *dev_id)
{
	...
	if (intmask & SDHCI_INT_CMD_MASK) {
        // printk("[PowerOn] %s: Got MMC interrupt, SDHCI_INT_CMD_MASK\n",mmc_hostname(host->mmc));
	...
	if (intmask & SDHCI_INT_DATA_MASK) {
        // printk("[PowerOn] %s: Got MMC interrupt, SDHCI_INT_DATA_MASK\n",mmc_hostname(host->mmc));
		sdhci_writel(host, intmask & SDHCI_INT_DATA_MASK,
			SDHCI_INT_STATUS);
		sdhci_data_irq(host, intmask & SDHCI_INT_DATA_MASK);
        // printk("[PowerOn] sdhci_irq: sdhci_data_irq return.\n");
	}
    ...
    {
        // printk("[PowerOn] %s: Got MMC interrupt, SDHCI_INT_CARD_INT\n",mmc_hostname(host->mmc));
		cardint = 1;
    }
...
out:
    ...
    // printk("[PowerOn] sdhci_irq: out.\n");
	/*
	 * We have to delay this as it calls back into the driver.
	 */
	if (cardint)
    {
        // printk("[PowerOn] sdhci_irq: mmc_signal_sdio_irq.\n");
		mmc_signal_sdio_irq(host->mmc);
    }

    // printk("[PowerOn] sdhci_irq: ack_irq.\n");
...
int sdhci_add_host(struct sdhci_host *host)
{
    ...
    // printk("[PowerOn] sdhci_add_host: call sdhci_reset(host, SDHCI_RESET_ALL) \n");
	sdhci_reset(host, SDHCI_RESET_ALL);
	...
	caps = (host->quirks & SDHCI_QUIRK_MISSING_CAPS) ? host->caps :
     ...
     // printk("[PowerOn] sdhci_add_host: sdhci caps: %p\n",caps);
	...
	if (host->flags & SDHCI_USE_ADMA) {
		 ...
		 */
         // // printk("[PowerOn] Allocate descriptors for ADMA\n");
    ...
    // printk("[PowerOn] sdhci_add_host: sdhci caps max_clk %d * 1,000,000\n",host->max_clk);
	    ...
	    mmc_card_is_removable(mmc))
    {
        ...
        // printk("[PowerOn] sdhci_add_host: sdhci mmc caps quirks: Need card detection polling - %s\n",(mmc->caps & MMC_CAP_NEEDS_POLL?"Yes":"No"));
    }
    ...
    // printk("[PowerOn] sdhci_add_host: sdhci_init calledS\n");
	sdhci_init(host, 0);
    ...
    // printk("[PowerOn] sdhci_add_host: mmc_add_host calledS\n");
	mmc_add_host(mmc);
