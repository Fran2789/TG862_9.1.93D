...
// UNIHAN ADD START PROD00207497
#include <linux/cat_l2switch_netdev.h>
// UNIHAN ADD END PROD00207497
...
// UNIHAN ADD START PROD00207497  
    int prob=atomic_read(&neigh->probes);
// UNIHAN ADD END PROD00207497
	write_lock(&neigh->lock);
...
// UNIHAN MOD START PROD00207497  
		} else if (time_before_eq(now,
					  neigh->used + neigh->parms->delay_probe_time) || (memcmp(neigh->dev->name, L2SW_NETDEV_DATA0, sizeof(L2SW_NETDEV_DATA0)-1)==0 && prob==0) ) {
// UNIHAN MOD END PROD00207497  
...
// UNIHAN ADD END PROD00207497
    ...
    /* ARRIS MODIFY BEGIN: arp can re-connect device - send netlink notify */
    /* NOTE : using NUD_REACHABLE, as NUD_CONNECTED will flood us with 
     * NO_ARP state multicast neighbors 
     */
	if (new & NUD_CONNECTED) {
		neigh_connect(neigh);
        if ( (new & NUD_REACHABLE) && (!(old & NUD_CONNECTED)) ) {
            notify = 1;
            // UNIHAN ADD START for PROD00203106
            if (!memcmp(neigh->dev->name, L2SW_NETDEV_DATA0, sizeof(L2SW_NETDEV_DATA0)-1)) 
            {
                atomic_set(&neigh->probes, 0);
            }
            // UNIHAN ADD END for PROD00203106
        }
    }
	else
		neigh_suspect(neigh);
