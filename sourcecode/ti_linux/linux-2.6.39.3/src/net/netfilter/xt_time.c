...
/*SERCOMM ADD START*/
int (*daylightsaving_hook)(void) = NULL;
EXPORT_SYMBOL(daylightsaving_hook);
/*SERCOMM ADD END*/
...
time_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
    ...
    /*the hook will only affect the xt_time.c*/
    if (daylightsaving_hook)
    {
        int tz_minuteswest = 0;
        tz_minuteswest = daylightsaving_hook();
        //printk("Do daylightsaving_hook, tz_minuteswest=%d\n", tz_minuteswest);
        //printk("Before stamp=%lld\n", stamp);
    	stamp -= 60 * tz_minuteswest;
        //printk("After stamp=%lld\n", stamp);
    }
    else
    {
