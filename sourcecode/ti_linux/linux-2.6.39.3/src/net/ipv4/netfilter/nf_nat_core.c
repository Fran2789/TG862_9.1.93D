...
#ifdef CONFIG_TI_PACKET_PROCESSOR
    /* packet processor addition 
      in case we have a drop session and a client is trying to receive a traffic with the same
      properties (source and destenation ip , source and destination ports) as the drop session , the session needs to be deleted */
