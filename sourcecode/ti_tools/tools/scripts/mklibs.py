...
while 1:
    ...
    needed_symbols.add(("DocsisEventIdNamesModulePtr___", 1))
    needed_symbols.add(("PacmEventIdNamesModulePtr", 1))
        ...
        if not symbol_provider.has_key(symbol):
            if not is_weak:
                if not uclibc or (symbol != "main"):
                    debug(DEBUG_VERBOSE, "No library provides non-weak " + symbol)
        else:
