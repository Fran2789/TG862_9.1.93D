...
using namespace std;
// UNIHAN ADD START , PROD00200920 
#define INTERFACE_INI_FILE "/var/tmp/gwconfig/8/rnddb.ini"
#define INTERFACE_FORMAT "l2sd0.%d" 
#define INTERFACE_PREFIX_LEN "dhcpv6prefixlen"
#define RNDDB_NUMBER_FORMAT "[%d]"
#define IPV6_BINARY_LENGTH 128
#define IPV6_EUI64_FORMAT_LENGTH 64 
#define RFC2526_MAX_BIT_LENGTH 121 // subtract anycast length "7"
// UNIHAN ADD END , PROD00200920 
...
TSrvOptIA_NA::TSrvOptIA_NA(char * buf, int bufsize, TMsg* parent)
	...
	MsgType = parent->getType();	// UNIHAN ADD , PROD00225555 
...
bool TSrvOptIA_NA::renew(SPtr<TSrvOptIA_NA> queryOpt, bool complainIfMissing)
{
    ...
    unsigned long Count=0;		// SERCOMM ADD
    ptrIA->firstAddr();
...
// SERCOMM ADD START
        ...
        while (pool = iface->getAddrClass()) {
            // This is not the pool you are looking for.
            if (pool->addrInPool(ptrAddr->get())) {
                ptrAddr->setTimestamp();
                optAddr = new TSrvOptIAAddress(ptrAddr->get(), ptrAddr->getPref(),ptrAddr->getValid(),
                                               this->Parent);
                SubOptions.append( (Ptr*)optAddr );
                Count++;
                Log(Debug) << "Renewed Address= " << *ptrAddr->get() << LogEnd;
            } else {
                Log(Debug) << "Renewed Address= " << *ptrAddr->get() << " is not in pool! Re-assign later!" << LogEnd;
            }
        }
// SERCOMM ADD END
    }
...
/*SERCOMM ADD START*/
/*
    1. Fix for CDRouter dhcpv6_server_43
    2. PASS When DHCPV6 Pool Chage, the CPE still can get the new range IP
*/
    SPtr<TOpt> subOpt;
    queryOpt->firstOption();
	
	// UNIHAN MOD START , PROD00225555
	if( queryOpt->MsgType == RENEW_MSG )
	{
	    while ((subOpt = queryOpt->getOption()) ) {
	        ...
	        if (!cli) {
	            ...
	            Log(Warning) << "Renewed Address= " << optAddr->getAddr()->getPlain() << " is not in pool! Set Ptime=0, Vtime=0 , Re-assign later!" << LogEnd;
	            optAddr = new TSrvOptIAAddress(optAddr->getAddr(), 0, 0, this->Parent);
	            SubOptions.append( (Ptr*)optAddr );
	        }
	    }
	}
	// UNIHAN MOD END , PROD00225555
/*SERCOMM ADD END*/


    if(!Count)	// SERCOMM ADD: No valid IP, re-assing later
        return false;
...
/*SERCOMM ADD START*/
/*
    1. Fix for CDRouter dhcpv6_server_52
*/
    SPtr<TOpt> subOpt;
    queryOpt->firstOption();
    while ((subOpt = queryOpt->getOption()) ) {
        ...
        if (!cli) {
            ...
            Log(Warning) << "Rebinded Address= " << optAddr->getAddr()->getPlain() << " is not in pool! Set Ptime=0, Vtime=0 , Re-assign later!" << LogEnd;
            optAddr = new TSrvOptIAAddress(optAddr->getAddr(), 0, 0, this->Parent);
            SubOptions.append( (Ptr*)optAddr );
...
bool TSrvOptIA_NA::assignRandomAddr(SPtr<TSrvMsg> queryMsg, bool quiet) {
    ...
    if (pool->clntSupported(ClntDuid, ClntAddr, queryMsg) &&
        ...
        while (safety < SERVER_MAX_IA_RANDOM_TRIES) {
            ...
            if (SrvAddrMgr().addrIsFree(candidate) && !SrvCfgMgr().addrReserved(candidate) && checkReservedSubnet(candidate, iface->getName() )) // UNIHAN MOD ,PROD00200920
                break;
...
// UNIHAN ADD START ,PROD00200920
/* Refer /var/tmp/gwconfig/8/rnddb.ini , 
 * the first interface is l2sd0 : [0] , l2sd0.2 : [1] ...  l2sd0.9 : [8] 
*/
int TSrvOptIA_NA::getIntferNumber( string name )
{
    int number = 0;
    sscanf( name.c_str() , INTERFACE_FORMAT , &number );
    if( number >= 2 )
    {
        return number - 1;
    }
    ...
    {
        /* Return 0 , if the interface is l2sd0 or others interface */
        return 0;
    }
}

int TSrvOptIA_NA::getPrefixSubnetLength( std::string iface_name )
{
    ifstream rnddb_file( INTERFACE_INI_FILE );
    if( !rnddb_file.is_open() )
    {
        Log(Debug) << "OPEN FAILED : "<< INTERFACE_INI_FILE << LogEnd;
        return false;
    }
    int number = getIntferNumber(iface_name);
    char section[8] = {0};
    ...
    sprintf( section , RNDDB_NUMBER_FORMAT , number );
    string section_str( section );

    while( getline( rnddb_file , line ) )
    {
        if( find_section == false )
        {
            found = line.find( section_str );
        ...
        {
            found = line.find( INTERFACE_PREFIX_LEN );
            if( found != string::npos )
            {
                found = line.find( "=" );
                if( found != string::npos )
                {
                    prefix_len =  line.substr( found + 2 );
                    rnddb_file.close();
                    return atoi( prefix_len.c_str() );
                }
            }
        }
    }
...
bool TSrvOptIA_NA::isDuplicateReservedSubnet( int length  , string addr )
{
    string reserved_subnet = "";
    string addr_reserved_subnet = "";
    int reserved_subnet_length = IPV6_BINARY_LENGTH - length;
    reserved_subnet = reserved_subnet.append( reserved_subnet_length , '0' );
    addr_reserved_subnet = addr.substr( length , reserved_subnet_length  );
    ...
    /* rfc 4291 rules
     ...
     +------------------------------------------------+----------------+
    */
    if( reserved_subnet == addr_reserved_subnet )
    {
        Log(Debug) << "IP Address is resvered." << LogEnd;
        return true;
    }

    if( length <= RFC2526_MAX_BIT_LENGTH )
    {
        /* rfc 2526
        ...
        */
        reserved_subnet = "";
        addr_reserved_subnet = "";
        reserved_subnet_length = RFC2526_MAX_BIT_LENGTH - length;
        addr_reserved_subnet = addr.substr( length , reserved_subnet_length  );
        reserved_subnet = reserved_subnet.append( reserved_subnet_length , '1' );

        if( length == IPV6_EUI64_FORMAT_LENGTH )
        {
            /* rfc 2526
...
string TSrvOptIA_NA::ConvertIPv6ToBinary( string ipv6_addr )
{
    int i = 0;
    int  prefix_dec[16] = {0};
    char section_binary[9] = {0};
    int Remainder = 0; 
    int Quotient = 0; 
    int offset = 0;
    string prefix = "";
    ...
    if (inet_pton(AF_INET6, ipv6_addr.c_str() , &result) == 1)
    {
        for( i = 0 ; i < 16 ; i++ )
        {
            memset( section_binary , '0' , sizeof( section_binary ) );
            section_binary[8] = '\0';
            prefix_dec[i] = result.s6_addr[i] ;
            offset = 0;
            Quotient = prefix_dec[i];
            while( Quotient != 0 )
            {
                ...
                if( Remainder == 0 )
                {
                    section_binary[7-offset] = '0'; 
                }
                else
                {
                    section_binary[7-offset] = '1'; 
                }
                offset++;
            }
            prefix.append( section_binary );
        }
    }
    else
    { 
        Log(Debug) << "IPv6 Format is not legal!" << LogEnd;
    }
    return prefix;
}

bool TSrvOptIA_NA::checkReservedSubnet(SPtr<TIPv6Addr> ipv6_addr ,std::string iface_name ) {
    
    int PrefixLength = getPrefixSubnetLength( iface_name );
    if( PrefixLength == - 1 )
    {
        Log(Debug) << "Get IPv6 prefix subnet length  failed !" << LogEnd;
        /* Return true. If can't get interface settings , let dibbler do original behavior */
        return true;
    }

    string addr = "" ;
    addr = ConvertIPv6ToBinary( ipv6_addr->getPlain() );
    if( addr.length() == 0 )
    {
        Log(Debug) << "Get IPv6 binary format failed !" << LogEnd;
        /* Return true. If can't get interface settings , let dibbler do original behavior */
        return true;
    }

    if( isDuplicateReservedSubnet( PrefixLength , addr ) == true )
    {
        return false; 
    }
