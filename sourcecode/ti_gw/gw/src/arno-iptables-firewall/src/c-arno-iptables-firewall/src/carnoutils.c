...
/* NAT_FORWARD_xxx is the largest format we have used in current, the format is "PORT-PORT>IpAddr~PORT-PORT\0",
   (PORT*4 + Ip + sign)*max rules = (5*4 + 15 + 5)*32 = 1280 */
#define PARSE_LEN 1280 /* UNIHAN ADD for PROD00218750 */
...
char *getfield(char *insrc, char *sep, int field_num)
{
    ...
    //if (src == NULL || strstr(src, sep) == NULL) /*SERCOMM MODIFY: fix bug, if there is no "sep" in "src", then return NULL*/
...
static int parse_value(char *value, char *r_value, int r_value_len)
{
    char *keyword;
    char *var_s;
    char *var_e;
    ...
    memset(r_value, '\0', r_value_len);
    keyword = strstr(tmp, "${");
    if(keyword == NULL) { /*no variable*/
        if(strlen(value) > r_value_len) {
            goto err_out;
        }
        else {
            strcpy(r_value, value);
            return 0;
        }
    }

    r_value_e = r_value;
    r_value_max_e = r_value + r_value_len;

    while( keyword != NULL) {
        len = keyword-tmp;
        if( (r_value_e + len) > r_value_max_e) {
            goto err_out;
        }
        memcpy(r_value_e, tmp, len); /*cp non-variable part*/
        r_value_e = r_value_e + len; /*move end of r_value_e*/
        tmp = keyword; /*move tmp to keyword, now tmp is at "$"*/

        /*handle variable*/
        /*only support variable:  ${XXXXX} */
        var_e = strchr(tmp, '}'); /*find end of variable*/
        if( var_e != NULL) { /*end of variable found*/
            if(var_e == keyword+2) {/*empty variable, "${}"*/
                tmp = tmp + 3; /*skip "${}", and do next run*/
            }
            else{ /*variable found*/
                var_s = keyword + 2; /*skip "${"*/
                *var_e = '\0'; /*add 0 to end of variable, so we can use var_s as a string*/
                var_value = getenv(var_s);
                ...
                if( var_value != NULL ) {
                    ...
                    memcpy(r_value_e, var_value, len); /*copy all remain*/
                    r_value_e = r_value_e + len; /*move end of r_value_e*/
                }
        ...
        else { /*no end of variable found, it's not variable and there is no variable remain*/
            /*no more variable, copy all remain*/
            len = strlen(tmp);
            if( (r_value_e + len) > r_value_max_e) {
                goto err_out;
            }
            memcpy(r_value_e, tmp, len); /*copy all remain*/
            r_value_e = r_value_e + len; /*move end of r_value_e*/
            tmp = tmp + len;
        }
        keyword = strstr(tmp, "${"); /*find next*/
    }
...
err_out:
    printf("ERROR: %s fail, value \"%s\" out of buffer\n", __FUNCTION__, value);
    memset(r_value, '\0', r_value_len); /*set 0 to r_value*/
    return 1;
}

static int setenv_line(char *env)
{
    char *name;
    char *value;
    int value_len=0;
    char v[PARSE_LEN];
    name = strtok_r(env, "=", &value);
    value_len = strlen(value);
    //printf("value_len=%d, value[0]=%c, value[value_len-1] =%c\n", value_len, value[0], value[value_len-1]);
    if( (value_len>0) && (value[0]=='\"') && (value[value_len-1] == '\"') ) { /*string with "", remove ""*/
        value[value_len-1] = '\0';
        value = value + 1;
    }

    parse_value(value, v, PARSE_LEN);
    
    //printf("name=%s, value=%s v=%s\n", name, value, v);
    return setenv(name,v,1);
}

void source_env(char *filename)
{
    int fd;
    int l;
    int i=0;
    ...
    if (stat(filename, &stat_buf) == 0)
    {
        ...
        file_buff = malloc(len + 11); /* to be on the safe side... */
        if ( file_buff == NULL ) {
            printf("\033[40m\033[1;31mERROR: CARNO unable to allocate memory in %s \033[0m\n", __FUNCTION__);
            return;
        }
        fd = open(filename, O_RDONLY);
        if (fd >= 0)
        {
            ...
            for (line = strtok_r(file_buff, "\n", &line_r);line != NULL; line = strtok_r(NULL, "\n", &line_r))
            {
                i++;
                /* Skip leading blanks */
                // UNIHAN ADD START for PROD00217367
                if (!multi_line)
                {
                  line += strspn(line, " ");
                }
                // UNIHAN ADD END for PROD00217367
                /* Comment? */
                if (line[0] == '#')
                {
                    continue;
                ...
                if( line[line_len-1] == '\\') { /*multi-line variable*/
                    line[line_len-1] = '\0';
                    if(multi_line == NULL) {
                        multi_line = malloc(line_len+1);
                        if(multi_line == NULL) {
                            printf("\033[40m\033[1;31mERROR: CARNO unable to allocate memory in %s \033[0m\n", __FUNCTION__);
                            return;
                        }
                        memcpy(multi_line, line, line_len);
                    }
                    else {
                        tmp = multi_line;
                        tmp_len = strlen(tmp);
                        multi_line = malloc(tmp_len + line_len +1);
                        if(multi_line == NULL) {
                            printf("\033[40m\033[1;31mERROR: CARNO unable to allocate memory in %s \033[0m\n", __FUNCTION__);
                            free(tmp);
                            return;
                        }
                        memcpy(multi_line, tmp, tmp_len);
                        memcpy(multi_line+tmp_len, line, line_len+1);
                        free(tmp);
                    }
                }
                else {
                    if(multi_line != NULL) { /*multi-line variable, last line*/
                        tmp = multi_line;
                        tmp_len = strlen(tmp);
                        multi_line = malloc(tmp_len + line_len +1);
                        if(multi_line == NULL) {
                        ...
                        memcpy(multi_line+tmp_len, line, line_len+1);
                        free(tmp);
                        //printf("multi-line variable : %s\n", multi_line);
                        /*set env*/
                        setenv_line(multi_line);
                        free(multi_line);
                        multi_line = NULL;
                    }
                    else { /*Single-line variable*/
                        //printf("single-line variable: %s\n", line);
                        setenv_line(line);
                    }
                }
        ...
        printf("\033[40m\033[1;31mWARNING: CARNO fail to load config file %s \033[0m\n", filename);
    }
}
