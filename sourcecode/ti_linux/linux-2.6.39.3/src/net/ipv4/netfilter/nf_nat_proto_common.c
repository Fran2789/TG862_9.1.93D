			       ...
			       u_int16_t *rover)
{
	...
	/* UNIHAN ADD START */
	if ((range->flags & IP_NAT_RANGE_PROTO_SPECIFIED) && !(range->flags & IP_NAT_RANGE_PROTO_RANDOM) &&
		(maniptype == IP_NAT_MANIP_DST)) {
		struct nf_conn_nat *nat;

		if ((nat = nfct_nat(ct)) && (nat->portfwd_position < range_size)) {
			*portptr = htons(min + nat->portfwd_position);
			*rover = off + nat->portfwd_position;
			return;
		}
	}
	/* UNIHAN ADD END */
