...
static int
ipt_psd_time_after_eq(clock_t currentTime, clock_t currentStamp)
{
   return currentTime - currentStamp >= 0;
}
/*
...
ipt_psd_match(const struct sk_buff *pskb,
	      ...
	      int *hotdrop)
#endif
static bool ipt_psd_match(const struct sk_buff *pskb, struct xt_action_param *par)
{
	struct iphdr *iph;
	...
	if (curr) {
		...
		if (now - curr->timestamp <= (psdinfo->delay_threshold*HZ)/100 &&
		    ipt_psd_time_after_eq(now, curr->timestamp)) {
...
#endif
static int ipt_psd_checkentry(const struct xt_mtchk_param *par)
{
...
/*  	} */
#if 0
	if (matchsize != XT_ALIGN(sizeof(struct ipt_psd_info))) {
		DEBUGP("PSD: matchsize %u != %u\n",
		       matchsize,
		       XT_ALIGN(sizeof(struct ipt_psd_info)));
		return 0;
	}
...
/*ipt_match is not avaliable in new kernel*/
/*This definition is already defined in ./linux/netfilter_ipv4/ip_tables.h*/
/*but the "__KERNEL__" always be defined, so this definition(ipt_match) is not defined in ./linux/netfilter_ipv4/ip_tables.h*/
