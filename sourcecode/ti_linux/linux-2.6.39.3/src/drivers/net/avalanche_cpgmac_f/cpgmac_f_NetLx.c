...
#ifdef CONFIG_SYSTEM_EXTERNALSWITCH
...
//static char* eth_mdio_num; //ARRIS REMOVE - INTEL ADD but un-used
#endif
...
#ifdef CONFIG_SYSTEM_EXTERNALSWITCH
//Read or Write broadcom switch register
/* BCM53115s define */
...
#define PSEUDO_PHY_ADDR             0x1e    /* Pseduo PHY address *//* Pseudo PHY MII registers */
...
static struct proc_dir_entry *gp_mdio_file = NULL;
static unsigned int sc_static_PhyNum=0;
static unsigned int sc_static_regadr=0;
static int    sc_static_val=0;
static int    sc_static_ret=0;

int bcm53115s_mdio_read(PHY_DEVICE *PhyDev,unsigned int pageNum ,unsigned int regadr)
{
	unsigned int cmd,res,val,max_retry=0;

    cmd = (pageNum << REG_PPM_REG16_SWITCH_PAGE_NUMBER_SHIFT) | REG_PPM_REG16_MDIO_ENABLE;
	_cpswHalCommonMiiMdioUserAccessWrite(PhyDev,REG_PSEUDO_PHY_MII_REG16,PSEUDO_PHY_ADDR,cmd);

	cmd = (regadr << REG_PPM_REG17_REG_NUMBER_SHIFT) | REG_PPM_REG17_OP_READ;

	_cpswHalCommonMiiMdioUserAccessWrite(PhyDev,REG_PSEUDO_PHY_MII_REG17,PSEUDO_PHY_ADDR,cmd);			
	do {
		res = _cpswHalCommonMiiMdioUserAccessRead(PhyDev,REG_PSEUDO_PHY_MII_REG17,PSEUDO_PHY_ADDR);
		// ARRIS MOD
		if ((res & (REG_PPM_REG17_OP_WRITE|REG_PPM_REG17_OP_READ)) == REG_PPM_REG17_OP_DONE)
		{
			break;
		}
		msleep(20);
	...
	cmd = _cpswHalCommonMiiMdioUserAccessRead(PhyDev,REG_PSEUDO_PHY_MII_REG24,PSEUDO_PHY_ADDR);
	val = (cmd&0xffff);

	cmd = _cpswHalCommonMiiMdioUserAccessRead(PhyDev,REG_PSEUDO_PHY_MII_REG25,PSEUDO_PHY_ADDR);
	val |= (cmd&0xffff) <<16;

	return val;
}

int bcm53115s_mdio_write(PHY_DEVICE *PhyDev,unsigned int pageNum ,unsigned int regadr,unsigned int value)
{
	unsigned int cmd,res,val,max_retry=0;
	val = value;
	...
	cmd = val>>0 & 0xffff;
	_cpswHalCommonMiiMdioUserAccessWrite(PhyDev,REG_PSEUDO_PHY_MII_REG24,PSEUDO_PHY_ADDR,cmd);

     	cmd = val>>16 & 0xffff;
	_cpswHalCommonMiiMdioUserAccessWrite(PhyDev,REG_PSEUDO_PHY_MII_REG25,PSEUDO_PHY_ADDR,cmd);

	cmd = 0;
	_cpswHalCommonMiiMdioUserAccessWrite(PhyDev,REG_PSEUDO_PHY_MII_REG26,PSEUDO_PHY_ADDR,cmd);
		
	cmd = 0;
	_cpswHalCommonMiiMdioUserAccessWrite(PhyDev,REG_PSEUDO_PHY_MII_REG27,PSEUDO_PHY_ADDR,cmd);

    	cmd = (regadr << REG_PPM_REG17_REG_NUMBER_SHIFT) | REG_PPM_REG17_OP_WRITE;
...
static int cpmac_p_write_mdio(struct file *fp, const char *buf, unsigned long count,void *data)
{
	char local_buf[31];
	int ret_val = 0;
	...
	hDDA = NETDEV_PRIV(p_dev);
   	PhyDev = (PHY_DEVICE *) DDC_cpmacGetPhyDev(hDDA->hDDC);
	sscanf(local_buf,"%x %x %x",&regadr,&pageNum,&value);

	printk("Page:%x Address:%x Value:%x\n",pageNum,regadr,value);

	if(value == 0x8888){
		val = bcm53115s_mdio_read(PhyDev,pageNum,regadr);	

		sc_static_PhyNum=pageNum;
		sc_static_regadr=regadr;
		sc_static_val=val;
		sc_static_ret=0;

        printk("Read Page:%x Address:%x Value:%x\n",pageNum,regadr,val);

	}else if(pageNum >= 0x200){
		pageNum-=0x200;
		_cpswHalCommonMiiMdioUserAccessWrite(PhyDev,regadr,pageNum,value);

		val = _cpswHalCommonMiiMdioUserAccessRead(PhyDev,regadr,pageNum);
		...
		printk("Write regadr=%x PhyNum=%x value=%x ret==%x\n",regadr,pageNum,value,val);

	}else if(pageNum >= 0x100){
		pageNum-=0x100;
		...
		printk("Read regadr=%x PhyNum=%x value=%x ret==%x\n",regadr,pageNum,value,val);
	}else if((regadr >= 0 )&&( pageNum >= 0) && (value>=0)){

		bcm53115s_mdio_write(PhyDev,pageNum,regadr,value);
		...
		printk("echo <register address> <page> <value> > /proc/avalanche/cpmac_mdio");
	}
	return ret_val;
}
static int cpmac_p_read_mdio(char *buf, char **start, off_t offset, int count, int *eof, void *data)
{
    int len = 0;
    len += sprintf(buf + len,"%s:\t %x\t %x\t %x\n",
		(sc_static_ret==0)?"read":"write",sc_static_PhyNum,sc_static_regadr,sc_static_val);
    return len;
}
// ARRIS ADD
static int cpmac_p_read_reg(bit32u regaddr, bit32u pageNum)
{
	struct net_device *p_dev = last_cpmac_device;
	CpmacNetDevice *hDDA;
	PHY_DEVICE *PhyDev;
    ...
    // internal PHY registers cannot be accessed via PSEUDO_PHY_ADDR; use faked out pagenum to identify these
	if(pageNum >= 0x100)
    {
		pageNum-=0x100;
		val = _cpswHalCommonMiiMdioUserAccessRead(PhyDev,regaddr,pageNum);
    }
    else
    {
        val = bcm53115s_mdio_read(PhyDev,pageNum,regaddr);	
    }
    return val;
}
static int cpmac_p_write_reg(bit32u regaddr, bit32u pageNum, bit32u value)
{
	struct net_device *p_dev = last_cpmac_device;
	CpmacNetDevice *hDDA;
	PHY_DEVICE *PhyDev;
    ...
    {
		pageNum-=0x200;
		_cpswHalCommonMiiMdioUserAccessWrite(PhyDev,regaddr,pageNum,value);
		val = _cpswHalCommonMiiMdioUserAccessRead(PhyDev,regaddr,pageNum);
    }
    else
    {
        val = bcm53115s_mdio_write(PhyDev,pageNum,regaddr,value);	
    }
    return val;
}
// END ARRIS ADD
...
#ifdef INCLUDE_SERCOMM_GW //Stop queue and tasklet before channel reopen
	netif_stop_queue(p_dev);
#ifdef TX_TASKLET_MODE
	tasklet_disable(&hDDA->tx_tasklet);
#endif
#ifdef RX_TASKLET_MODE 
	tasklet_disable(&hDDA->rx_tasklet);
#endif
#endif
...
#ifdef INCLUDE_SERCOMM_GW //Wake up queue and enbale tasklet
	netif_wake_queue(p_dev);
#ifdef TX_TASKLET_MODE
	tasklet_enable(&hDDA->tx_tasklet);
#endif
#ifdef RX_TASKLET_MODE 
	tasklet_enable(&hDDA->rx_tasklet);
#endif
#endif
...
#if 0 /* THB - This does not compile - why is it here */
#ifdef CONFIG_SYSTEM_EXTERNALSWITCH // ARRIS ADD
                phyParams.data = cpmac_p_read_reg( phyParams.regAddr, phyParams.phyNum );
#else
#endif // END ARRIS
#ifdef CONFIG_SYSTEM_EXTERNALSWITCH // ARRIS ADD
                cpmac_p_write_reg( phyParams.regAddr, phyParams.phyNum, phyParams.data );
#else
#endif // END ARRIS
#endif
...
#ifdef CONFIG_SYSTEM_EXTERNALSWITCH //Create proc entry
	gp_mdio_file =
            create_proc_entry("avalanche/cpmac_mdio", 0644, NULL);
        if (gp_mdio_file) {
            gp_mdio_file->read_proc = cpmac_p_read_mdio;
            gp_mdio_file->write_proc = cpmac_p_write_mdio;
        }
#endif
...
#ifdef CONFIG_SYSTEM_EXTERNALSWITCH //Remove proc entry
    if (gp_mdio_file)
        remove_proc_entry("avalanche/cpmac_mdio", NULL);
#endif
