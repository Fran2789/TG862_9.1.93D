...
#else
    ulong load_addr;                    /* Default Load Address, assigned in board.c */
#endif
...
#ifdef CONFIG_EXTENDED_FLASH
    ulong retlen;
    int switch_res = process_flash_bank(NULL, data, sizeof(ulong*), &retlen, 1);
    if (switch_res == EMM_HIGH)
    {
        /* get length of script */
	    len_ptr = (ulong *)((uchar*) data - FLASH_BORDER_OFFSET_16MB);
    }
    else if (switch_res == EMM_LOW)
    {
        /* Since data is aligned to 4 bytes offset, this should not happen */
        if (retlen != sizeof(ulong*))
            printf("error: %s:%d read %d bytes from %d in low bank \n", 
            __FUNCTION__, __LINE__, retlen, sizeof(ulong*));
...
#ifdef CONFIG_EXTENDED_FLASH
    /* Set both (OMER TEMP -Which both) flash CS0 and CS1 to low area because linux expects always low */
    process_flash_bank(NULL, CFG_FLASH1_BASE, 0, NULL, 1);
    process_flash_bank(NULL, CFG_FLASH2_BASE, 0, NULL, 1);
#endif
	puts ("OK\n");
	SHOW_BOOT_PROGRESS (7);
...
{
#ifdef CONFIG_EXTENDED_FLASH
    image_header_t header = {0}, *hdr_tmp;
    hdr_tmp = hdr;
    uint8_t ih_type;
    ulong retlen;
    int switch_res;
    /*Copy header in temp variable, in case when original header 
    saved cross two banks, it is easier to read it correctly 
    (at least first 6 fields)*/
    memmove (&header, (char *)hdr, sizeof(image_header_t));
    hdr = &header;
#endif
#if (CONFIG_COMMANDS & CFG_CMD_DATE) || defined(CONFIG_TIMESTAMP)
    ...
    /*Save type of header*/
    ih_type = hdr->ih_type;
    /*Restore pointer to original header*/ 
    hdr = hdr_tmp;

    if (ih_type == IH_TYPE_MULTI) {
#else
	if (hdr->ih_type == IH_TYPE_MULTI) {
		...
		puts ("   Contents:\n");
#ifdef CONFIG_EXTENDED_FLASH
        switch_res = process_flash_bank(NULL, len_ptr, sizeof(*len_ptr), &retlen, 1);
        if (switch_res == EMM_HIGH)
        {
            len_ptr =(ulong*)((uchar*)len_ptr - FLASH_BORDER_OFFSET_16MB);
            debug("%s:%d switched to high\n", __FUNCTION__, __LINE__);
        }
        else if (switch_res == EMM_LOW)
        {
            /* Since len_ptr value aligned to 4 bytes offset, this should not happen */
            if (sizeof(*len_ptr) != retlen)
                printf ("error: %s:%d read %d bytes from %d in low bank\n", 
                __FUNCTION__, __LINE__, retlen, sizeof(*len_ptr));
        }
    	...
    	for (i=0; len; ++i) {            
            ...
            debug("len_ptr=0x%x size=%d\n", len_ptr, sizeof(*len_ptr));
            
            ++len_ptr;
        
            /*If we currently in low bank, we have to check next len_ptr*/
            if (switch_res != EMM_HIGH)
            {
                switch_res = process_flash_bank(NULL, len_ptr, sizeof(*len_ptr),
                                                                    &retlen, 1);
                if (switch_res == EMM_HIGH)
                {
                    len_ptr =(ulong*)((uchar*)len_ptr - FLASH_BORDER_OFFSET_16MB);
