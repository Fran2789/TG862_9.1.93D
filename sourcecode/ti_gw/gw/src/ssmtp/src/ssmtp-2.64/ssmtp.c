...
/*SERCOMM ADD START*/
#include <net/if.h>
static char g_bindInterface[IFNAMSIZ];
/*SERCOMM ADD END*/
...
void log_event(int priority, char *format, ...)
{
	...
	if((fp = fopen("/var/tmp/ssmtp.log", "a")) != (FILE *)NULL) { //SERCOMM MODIFY
		(void)fprintf(fp, "%s\n", buf);
		(void)fclose(fp);
	}
	else {
		(void)fprintf(stderr, "Can't write to /var/tmp/ssmtp.log\n"); //SERCOMM MODIFY
	}
#endif
...
/*SERCOMM ADD START*/
    ...
    snprintf(ifr.ifr_name, sizeof(ifr.ifr_name), g_bindInterface);
/*SERCOMM ADD END*/
	...
	for (ai = ai0; ai; ai = ai->ai_next) {
        ...
        if (strlen(g_bindInterface) > 0) {
            if (setsockopt(s, SOL_SOCKET, SO_BINDTODEVICE, (void *)&ifr, sizeof(ifr)) < 0)  {
    		    log_event(LOG_ERR, "SO_BINDTODEVICE Error\n");
                continue;
            }
        }
...
*/
            ...
            /*SERCOMM ADD START*/
			case 'I':
                memset(g_bindInterface, 0x00, sizeof(g_bindInterface));
                strcpy(g_bindInterface, argv[i+1]);
				add++;
				goto exit;
            /*SERCOMM ADD END*/
