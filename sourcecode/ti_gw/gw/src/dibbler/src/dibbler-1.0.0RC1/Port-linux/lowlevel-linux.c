...
/* Sercomm Add for PROD00219451 */
        ...
        */
     {
        int msgtype = 0;
        if(pktinfo == NULL)
                printf("%s[%d]: pktinfo == NULL\n",__func__,__LINE__);

        msgtype = buf[0];
        if (!IN6_IS_ADDR_MULTICAST(&pktinfo->ipi6_addr)&&
           (msgtype == SOLICIT_MSG || msgtype == CONFIRM_MSG ||
             msgtype == REBIND_MSG || msgtype == INFORMATION_REQUEST_MSG))
        {
                printf("%s[%d]: Ignore this unicast packet(include solicit, confirm, rebind, information_req)\n"
                        ,__func__,__LINE__);
                return LOWLEVEL_ERROR_UNSPEC;

        }
     }
/* End Sercomm for PROD00219451 */
